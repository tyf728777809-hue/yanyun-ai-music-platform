...
执行；运行时已阻止在默认 `sync` 模式下跑真实 Provider。首次单作品真实音乐 smoke 可使用
`scripts/smoke/dreammaker-real-music-smoke.sh`，但必须额外设置
`ALLOW_DREAMMAKER_REAL_SMOKE=1`，它会真实调用 DreamMaker，不能作为普通自动化测试运行。
