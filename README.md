...
执行；运行时已阻止在默认 `sync` 模式下跑真实 Provider。首次单作品真实音乐 smoke 优先使用
`scripts/smoke/dreammaker-real-music-stack-smoke.sh`，它会启动 worker/API、执行一次真实 Provider smoke 并自动清理进程；
若 worker/API 已手动启动，可改用 `scripts/smoke/dreammaker-real-music-smoke.sh`。两者都必须额外设置
`ALLOW_DREAMMAKER_REAL_SMOKE=1`，会真实调用 DreamMaker，不能作为普通自动化测试运行。
...
`IMAGE_REAL_CALLS_ENABLED=true` 和 `DREAMMAKER_REAL_CALLS_ENABLED=true` 后，会通过 DreamMaker
任务式客户端调用 `gpt-image-2`，并把供应商图片 URL 导入平台对象存储后再进入发布包。真实联调按
...
已包含 `image2_guard`，用于展示 Image 2 与 DreamMaker 真实调用开关和 AK/SK 配置状态。
...
- 真实 DeepSeek、Image 2、MiniMax 手动联调；Suno 已执行一次单作品真实 smoke，但 DreamMaker 创建任务阶段返回 HTTP 403，需确认账号权限、`X-Access-Token` 和模型配置后再复测。
