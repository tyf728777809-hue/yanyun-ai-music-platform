...
真实 smoke 脚本必须额外设置 `ALLOW_DREAMMAKER_REAL_SMOKE=1` 才会调用 DreamMaker。
...
- 真实 DeepSeek、Image 2、MiniMax 手动联调；Suno via DreamMaker 已执行一次单作品真实 smoke，但 DreamMaker 创建任务阶段返回 HTTP 403，初步判断与公司内网/账号权限有关。当前改走 Yunwu 公网联调路径，尚未执行真实成功 smoke。
