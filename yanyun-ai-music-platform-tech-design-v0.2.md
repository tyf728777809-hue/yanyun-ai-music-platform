...
| 文档版本 | v0.2 |
| 文档日期 | 2026-06-05 |
| 输入文档 | 《燕云 AI 作曲平台 PRD v0.3》 |
| 目标阶段 | 本地完整跑通 → 公司开发接入社区系统 → 公司服务器部署 |
| 技术定位 | 正式商用级平台架构，不按 Demo 或临时活动脚手架设计 |
| 核心模型 | DeepSeek V4 Pro、MiniMax Music-2.6、Image 2 |
| 核心产物 | 16:9 MP4 歌词视频发布包 |
| 用户侧范围 | React + Vite + TypeScript 响应式 Web，移动端优先，兼容 PC Web |
| 当前不包含 | 社区账号真实接入、审核真实接入、发布真实接入、分享真实接入、任务权益真实接入、社区点赞收藏评论 |

---

## 1. 技术目标

本技术方案的目标是把 PRD v0.3 转换为可研发、可本地跑通、可部署、可接入公司系统的技术基线。
...
8. **用户侧多端可用**：提供移动端优先、兼容 PC Web 的响应式用户侧 Web 应用。
9. **商用级栈不降级**：核心技术栈在第 1 批完成工程预置和本地环境纳入，业务能力按开发批次逐步启用。
...
React + Vite 负责用户侧创作、确认、生成中、成品预览和作品列表
...
```mermaid
flowchart TD
  U[移动端 H5/WebView / PC Web] --> WEB[apps/web]
  WEB --> API[music-api]
...
6. `apps/web` 只调用 `music-api`，不直接访问数据库、对象存储或 Provider。
...
10. ImageProvider.generateCover
11. If Image 2 failed: use default cover
12. Build lyric timeline
13. RenderWorker.renderVideo
14. Store MP4 and metadata
15. Build publish package
16. ModerationAdapter.preCheckPublishPackage，当前本地 MockAllow
17. QuotaAdapter.commit，MP4 发布包可用后扣减主权益
18. Update work status to GENERATED and package_status = PACKAGE_READY
```
...
| 视频失败 | keep audio and cover；status FAILED；allow rerender；主权益不 commit，可按配置继续锁定或释放 |
| package build 失败 | retry package build，不重新出歌 |
| publish package pre-check failed | package_status = PACKAGE_BLOCKED；release quota；保留排查信息 |
...
```json
{
  ...
  "polish_remaining": 2,
  "quota_hint": {
    "lock_required_on_confirm": true,
    "commit_timing": "PACKAGE_READY"
  },
  "available_actions": ["CONFIRM", "POLISH", "COPY_AS_NEW_DRAFT"]
}
```

当状态为 `GENERATING`、`GENERATED` 或 `FAILED` 时，还应返回：

```json
{
  "generation_stage": "MUSIC_GENERATING | COVER_GENERATING | VIDEO_RENDERING | PACKAGE_BUILDING | COMPLETED | FAILED",
  "package_status": "PACKAGE_NOT_READY | PACKAGE_READY | PACKAGE_FETCHED | PACKAGE_EXPIRED | PACKAGE_BLOCKED",
  "failure_code": "string",
  "failure_message": "string",
  "available_actions": ["RERENDER_VIDEO", "REFRESH_PACKAGE_URL", "COPY_AS_NEW_DRAFT"],
  "publish_handoff_hint": "作品已生成，可进入后续发布流程"
}
```
...
接口路径为 v0.1 草案，后续在 `docs/api/openapi-v0.1.yaml` 中冻结。
...
```text
数据库乐观锁 version 字段，关键状态写入可配合 SELECT FOR UPDATE
...
2. 启动 Temporal workflow 与数据库事务之间使用状态补偿；进入真实模型链路前补可靠 Outbox 或等价补偿机制。
...
```text
music-api: 2 副本起
web: 2 副本起，静态资源可接入公司 CDN 或 Web 容器
...
第 1 批必须纳入完整商用级组件边界和本地基础设施，但不要求完成业务深度集成。后续批次按领域能力逐步启用和验收。
...
6. Web 前端页面与状态流设计 v0.1
7. Codex 第 1 批仓库初始化任务说明
```

推荐下一步先做：

```text
OpenAPI 接口契约 v0.1 + Codex 第 1 批任务说明 + apps/web scaffold
```
