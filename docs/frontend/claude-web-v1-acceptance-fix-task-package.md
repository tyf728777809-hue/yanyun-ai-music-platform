...
本任务包原本用于收敛 Claude Web v1 的契约和验收缺口。用户明确要求后，当前缺口已由本 Agent 直接修复并纳入 Git 快照。

已验证：

- `npm test`：8 个测试文件、28 个测试通过。
- `npm run typecheck`：通过。
- `npm run build`：通过。
- 390px / 1440px Playwright smoke：通过。
- 真实后端模式 `npm run smoke:real-backend`：通过，覆盖灵感成歌、润色/续写次数、第三次 409 友好提示、确认出歌、发布交接、作品列表和失败页重试恢复。

本文件作为历史任务包和验收依据保留，后续不再按“待修复”状态执行。下面“当前缺少/当前成品页”等措辞均保留为任务包创建时的原始缺口描述。
