...
当前代码状态：截至 2026-06-06，`CoverGenerationService` 默认注册 `MockCoverGenerationService`；当 `IMAGE_PROVIDER=image2` 或 `IMAGE_REAL_CALLS_ENABLED=true` 时注册 `DreamMakerImage2CoverGenerationService`。Image 2 通过 DreamMaker AK/SK 和 JWT 接入，默认 `app_name=gpt-image-2`、`sub_app_name=gpt-image-2`、`model=gpt-image-2`。
...
- 单次联调只打开 Image 2；DeepSeek、Suno、MiniMax 和公司 Adapter 继续 Mock 或关闭。虽然需要 `DREAMMAKER_REAL_CALLS_ENABLED=true`，但 `MUSIC_PROVIDER=mock` 且请求不传 `music_provider=suno|minimax`，不会触发真实音乐。
...
export IMAGE2_OUTPUT_FORMAT=png
```
...
- 缺失 DreamMaker AK/SK、`DREAMMAKER_REAL_CALLS_ENABLED=true` 或 `IMAGE_REAL_CALLS_ENABLED=true`：必须在外部 HTTP 请求前失败，不得发出真实请求。
...
- DreamMaker AK/SK 或公司 Secret 注入方式。
- 本轮联调样本数量、成本止损和回滚窗口。
