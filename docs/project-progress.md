...
- `prototypes/Claude-web-v1` 已完成当前验收修复和真实后端模式 UI smoke；后续需决定是否把该原型合并/迁移到正式 `apps/web`，还是继续作为独立原型交给外部前端实现者迭代。
...
2. 若继续推进本地完整闭环，下一步应把真实后端 UI smoke 脚本化，避免只依赖手动 Playwright 操作。
3. 针对 `RENDER_WORKER_MODE=local-process` 做前端 UI 端到端 smoke：确认出歌后由 Java 调用 render-worker，检查页面视频地址、发布包 URL、对象存储文件和 MP4 `ffprobe`。
