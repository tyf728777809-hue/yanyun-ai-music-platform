...
2. 针对 `RENDER_WORKER_MODE=local-process` 做前端 UI 端到端 smoke：确认出歌后由 Java 调用 render-worker，检查页面视频地址、发布包 URL、对象存储文件和 MP4 `ffprobe`。
3. 准备真实模型受控联调清单：DreamMaker Suno / MiniMax、DeepSeek、Image 2 均需明确联调窗口、成本止损、失败码记录和密钥注入方式。
4. 强化公司 Adapter 部署交接验收清单，把 `/internal/integration-readiness` 的阻塞项、环境变量、替换接口和公司侧 smoke 串成一张可交付 checklist。
