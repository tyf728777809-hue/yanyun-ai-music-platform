...
- Phase 1 的第一块已落代码：新增 `agent_runs` 与 `AgentRunRecorder`，并接入写词链路的 `LyricsAgent` Mock 审计；后续继续补其他 Agent 合约和真实模型硬开关。
...
- AI 多 Agent v0.1 已确定方向，`agent_runs` 表和写词链路 `LyricsAgent` 审计已落地；后续还需补 `CreativeBriefAgent`、`MusicPromptAgent`、`CoverPromptAgent`、`QualityEvaluationAgent` 和 `ModerationAgent` 的 Mock 合约。
...
2. 继续按 `docs/specs/ai-multi-agent-creative-pipeline-v0.1.md` 补齐 `CreativeBriefAgent`、`MusicPromptAgent`、`CoverPromptAgent`、`QualityEvaluationAgent` 和 `ModerationAgent` 的 Mock 合约与 `agent_runs` 审计。
