...
2026-06-17 03:20 CST 按用户要求推进写歌词 Agent v0.9.1 通用判断力收束：不再为寒香寻、九流门、失败者尊严、反救世主等单个题材继续堆专属规则，而是把 CraftPlan 抽象成通用作词判断：用户强表达优先、具体胜过正确、矛盾胜过顺滑、副歌必须有功能、知识库服务歌曲、曲风不改变世界、粗粝不等于粗口。`RealDeepSeekLyricsCraftPlanner` 已升级到 `v0.9.1` / `lyrics.craft.plan.v9.1` / template version `10`，`LyricsAgent` 继续兑现 `selected_device + selected_angle + chorus_mechanism`，但移除了“对某些题材如何写”的硬编码句式；`MockLyricsCraftPlanner`、`DefaultLyricsGenerationService` 和相关测试同步升级。A/B 脚本默认当前版本改为 `B_v0.9.1`，支持 `LYRICS_AGENT_AB_CASES_PATH` 指定样本集，并在 ignored 的本地报告中输出 `latency_summary`，方便判断 CraftPlan 多一次 DeepSeek 文本调用的耗时成本。新增通用压力测试集 `knowledge-base/lyrics-agent-v0.9.1-eval-cases.json`，覆盖用户强短语、失败者尊严、反宏大守护、轻快但难受、开放曲风边界、粗粝但不低俗、普通人不英雄化、旧铃声等 12 条真实玩家输入。验证通过：脚本 `py_compile`、v0.9.1 样本 JSON 格式、A/B plan dry-run、Java `:modules:creative-agent:test :modules:deepseek:test :modules:lyrics:test`、相关模块 `spotlessCheck` 和 `git diff --check`。本轮未调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker，未生成音乐、封面或视频。
