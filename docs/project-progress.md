...
4. Image2 真实封面联调当前公网先用 `TARGET=wellapi-image2 MODE=plan scripts/smoke/real-model-controlled-smoke.sh` 和 `TARGET=wellapi-image2 MODE=preflight scripts/smoke/real-model-controlled-smoke.sh` 确认路线，保持 `MUSIC_PROVIDER=mock` 后再用双重开关执行 WellAPI 单作品封面 smoke；生产目标 DreamMaker Image2 路径仍按 `docs/runbook/image2-controlled-real-integration.md` 切换 `IMAGE2_BACKEND=dreammaker` 后验证。
