...
- Phase 1 已落两块代码：新增 `agent_runs` 与 `AgentRunRecorder`，接入写词链路的 `LyricsAgent` Mock 审计；新增 `MusicPromptAgent` v0.1 Mock 合约并在确认出歌前接入 `SongProductionWorkflow`。
...
- AI 多 Agent v0.1 已确定方向，`agent_runs` 表、写词链路 `LyricsAgent` 审计和 `MusicPromptAgent` Mock 合约已落地；后续还需补 `CreativeBriefAgent`、`CoverPromptAgent`、`QualityEvaluationAgent` 和 `ModerationAgent` 的 Mock 合约。
...
2. 继续按 `docs/specs/ai-agent-orchestration-engineering-design-v0.1.md` 补齐 `CreativeBriefAgent`、`CoverPromptAgent`、`QualityEvaluationAgent` 和 `ModerationAgent` 的 Mock 合约与 `agent_runs` 审计。
