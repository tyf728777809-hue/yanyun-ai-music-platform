...
更新时间：2026-06-13 19:25 CST

## 当前阶段

项目已完成第 8 批 MinIO/S3 发布包强化，并已按用户要求在前端并行开发期间推进第 10 批公司 Adapter 接入与部署交接准备和第 11 批 render-worker 本地进程调用边界：数据库 migration、Work 领域状态机、Mock Adapter 边界、OpenAPI v0.1 主路径 API、本地 Mock 作曲与发布包、DreamMaker / Suno / MiniMax 受控真实调用实现、Outbox v0.1、API outbox 到独立 Temporal worker 的编排边界、真实音乐 Provider 运行时硬保护、DeepSeek / Prompt / Lyrics 写词边界、OpenAI 兼容 `RealDeepSeekLyricsClient`、受控燕云创作知识库边界、CoverGenerationService / VideoRenderService 媒体生成边界、DreamMaker Image2 与 WellAPI Image2 封面真实客户端、render-worker 本地 16:9 MP4 样例渲染、Java 可配置调用 render-worker CLI 的本地进程模式、发布包 JSON 的 local / S3-MinIO 可切换对象存储，以及内部公司接入 readiness 报告均已落地。2026-06-13 用户确认不做联网搜索，但需要燕云创作知识库补足公测后角色、剧情、地域、玩法和玩家语境；项目口径已从“完全取消知识库”更新为“受控、版本化、官方资料优先、实体优先 + pgvector 辅助的创作知识库”，默认仍可 disabled。2026-06-07 已执行真实 DeepSeek、Yunwu Suno、WellAPI Image2 单项 smoke，并完成公网真实完整体验样本；2026-06-11 又用 DeepSeek v4Pro + Yunwu Suno `chirp-fenix` / v5.5 + WellAPI Image2 + local-process MP4 跑通作品 `20fa149f-d5b1-40b2-9b22-8561148158db` 到 `GENERATED / PACKAGE_READY`，MP4 已验证 H.264 video + AAC audio 双轨。Yunwu timestamped lyrics 已用同一 `chirp-fenix` 样本验证，但当前供应商 base URL 未暴露可用 JSON 时间轴接口：默认路径返回 404，若干 `/suno/*` 探测路径返回 HTML。DreamMaker 音乐与 DreamMaker Image2 仍保留为正式生产目标；当前公网环境下 DreamMaker Suno 曾返回 HTTP 403，用户判断可能需要公司内网或供应商权限。公司账号、审核、权益、社区发布、分享和生产部署形态仍由公司系统接入或决策；所有真实外部调用仍需显式开关和安全注入凭据。
