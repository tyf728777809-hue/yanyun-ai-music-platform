...
- DeepSeek 真实受控联调文档、安全规则和验收清单已补齐；真实 API 协议、模型参数、失败码、限流策略、计费口径、真实客户端实现和联调窗口仍待确认。
- Image 2 真实受控联调文档、安全规则和验收清单已补齐；真实 API 细节、素材规范、对象存储导入、日志与数据留存规范、真实客户端实现和联调窗口仍待确认。
...
- Outbox v0.1 与 Temporal v0.1 已落地并可本地验证；真实模型链路前的 activity 细化设计、step audit 表、记录型 Mock step activity、受控分步 workflow 验证路径和 worker 显式开关已补齐。`stepwise-recording` 当前只验证 Temporal step audit，不产出发布包；主链路验收仍使用 legacy 或同步 Mock 模式。
...
2. 在用户提供 DeepSeek URL / API Key / 协议文档后，先实现 `RealDeepSeekLyricsClient` 硬开关、超时、失败码映射和脱敏日志，再按 `docs/runbook/deepseek-controlled-real-integration.md` 做单样本联调。
3. 在用户提供 Image 2 URL / API Key / 协议文档后，先实现 `RealImage2CoverGenerationService` 硬开关、对象存储导入、默认封面兜底、超时、失败码映射和脱敏日志，再按 `docs/runbook/image2-controlled-real-integration.md` 做单样本联调。
4. 按 `docs/specs/temporal-activity-decomposition-v0.1.md` 继续推进分步 activity：下一步先执行 `stepwise-recording` 本地 Temporal smoke，确认 outbox 成功和 `generation_job_steps` 写入；随后再让分步 activity 逐步接入真实音乐、封面、视频和发布包状态推进。
5. 按 `docs/checklists/local-commercial-delivery-audit-2026-06-06.md` 的未完成项逐项推进或交给公司确认。
6. 公司开发确认真实账号、审核、权益、发布、分享协议后，按 `docs/handover/company-adapter-deployment-handoff-v0.1.md` 替换 Mock Adapter，并用 `/internal/integration-readiness` 做部署前检查。
7. 在明确联调窗口和止损规则后，按 `docs/runbook/dreammaker-controlled-real-integration.md` 手动执行 Suno / MiniMax 各 1 次真实成功路径；不得把密钥、JWT 或用户 token 写入仓库、日志或测试。
