...
2. 若用户要测真实 AI 出歌，先按 `docs/runbook/dreammaker-controlled-real-integration.md` 启动 outbox + Temporal worker，并优先用 `REAL_PROVIDER=suno` 执行单作品真实音乐 smoke；不得在默认 `sync` 模式或无成本止损时运行。
