...
- Outbox v0.1 与 Temporal v0.1 已落地并可本地验证；真实模型链路前的 activity 细化设计、step audit 表和记录型 Mock step activity 已补齐，后续实现时需把当前单一 activity 委托拆成更细粒度、可幂等重试的音乐生成、封面、视频、发布包等活动。
...
4. 按 `docs/specs/temporal-activity-decomposition-v0.1.md` 继续推进分步 activity：先在测试 workflow / worker 中接入 `RecordingSongProductionStepActivities` 验证 step audit，再逐步拆音乐、封面、视频和发布包步骤。
