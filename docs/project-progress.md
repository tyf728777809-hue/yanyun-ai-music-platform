...
- Outbox v0.1 与 Temporal v0.1 已落地并可本地验证；真实模型链路前的 activity 细化设计、step audit 表、记录型 Mock step activity 和受控分步 workflow 验证路径已补齐。后续若要替换生产 worker，必须先加配置开关和本地 smoke，不得直接覆盖当前稳定单 activity 主链路。
...
4. 按 `docs/specs/temporal-activity-decomposition-v0.1.md` 继续推进分步 activity：下一步应加显式配置开关或独立 task queue，在本地受控模式下注册 `StepwiseSongProductionWorkflow` 和 `RecordingSongProductionStepActivities`，再逐步拆真实音乐、封面、视频和发布包步骤。
