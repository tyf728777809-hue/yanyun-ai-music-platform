...
- 仍未完成项已归类到 `docs/checklists/local-commercial-delivery-audit-2026-06-06.md`：OpenAPI 自动契约对拍、真实 Suno/MiniMax、真实 DeepSeek、真实 Image 2、公司五类 Adapter 替换、正式 `apps/web` 承接策略。
...
- 本轮只新增和更新 Markdown 文档，未运行真实模型调用、未修改业务代码。
...
- AI 多 Agent v0.1 已确定方向，但尚未落代码；后续需要确认是否新增独立 `agent_runs` 表，或先复用/扩展 `provider_calls` 记录 Agent 调用审计。
...
1. 补 OpenAPI v0.1 与后端响应的契约对拍脚本，先覆盖作品详情、作品列表、失败响应和发布包接口。
2. 决定 `prototypes/Claude-web-v1` 的工程去向：继续独立原型、并入正式 `apps/web`，或作为外部前端实现者的参考版本。
3. 按 `docs/specs/ai-multi-agent-creative-pipeline-v0.1.md` 设计 Agent Runtime / Agent 合约和 Mock Agent，实现前先决定 `agent_runs` 审计落库方案。
4. 准备真实模型受控联调清单：DreamMaker Suno / MiniMax、DeepSeek、Image 2 均需明确联调窗口、成本止损、失败码记录和密钥注入方式。
