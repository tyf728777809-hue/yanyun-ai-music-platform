...
| 2026-06-12 05:38 CST | 增加公网试用成本保险丝 | 为 `MockQuotaAdapter` 增加 `yanyun.quota.mock-max-generate-locks` 配置，默认仍为 999，不改变普通本地开发；公网临时试用环境将设置为 20 次真实出歌锁定上限，超过后在 worker 的权益锁定阶段返回 `QUOTA_LOCK_FAILED`，避免链接转发后无限调用真实模型。验证通过：`:modules:quota:test`、`:apps:music-api:compileJava`、`:apps:music-worker:compileJava`、`spotlessCheck`、`git diff --check` |
