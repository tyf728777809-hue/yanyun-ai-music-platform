...
项目已完成第 8 批 MinIO/S3 发布包强化，并已按用户要求在前端并行开发期间推进第 10 批公司 Adapter 接入与部署交接准备和第 11 批 render-worker 本地进程调用边界：数据库 migration、Work 领域状态机、Mock Adapter 边界、OpenAPI v0.1 主路径 API、本地 Mock 作曲与发布包、DreamMaker / Suno / MiniMax 受控真实调用实现、Outbox v0.1、API outbox 到独立 Temporal worker 的编排边界、DreamMaker 真实 Provider 硬开关、DeepSeek / Knowledge / Prompt / Lyrics 写词边界、OpenAI 兼容 `RealDeepSeekLyricsClient`、CoverGenerationService / VideoRenderService 媒体生成边界、DreamMaker Image2 封面真实客户端、render-worker 本地 16:9 MP4 样例渲染、Java 可配置调用 render-worker CLI 的本地进程模式、发布包 JSON 的 local / S3-MinIO 可切换对象存储，以及内部公司接入 readiness 报告均已落地。2026-06-07 已执行一次真实 Suno 单作品 smoke，链路触达 DreamMaker 创建任务阶段但返回 HTTP 403；本地编排、失败收口、权益释放、provider_call 记录和进程清理正常。真实 MiniMax、真实 DeepSeek、真实 Image 2 或公司系统调用仍未执行；所有真实外部调用仍需显式开关和安全注入凭据。
...
- Suno 和 MiniMax 的 DreamMaker run/status 接入骨架、JWT 鉴权、真实调用硬开关和受控联调文档已实现；Suno 首次真实 stack smoke 已触达 DreamMaker 创建任务阶段但返回 HTTP 403，需确认 AK/SK 权限、`X-Access-Token` 要求和 `app/sub_app/model` 配置；MiniMax 尚未真实调用。非零错误码样本、失败任务响应样本、限流/轮询策略、音频 URL 过期规则和计费口径仍待真实联调确认。
- DeepSeek 真实客户端已实现并默认关闭；真实 API Key、失败码、限流策略、计费口径、内容安全要求和联调窗口仍待确认。
- Image 2 DreamMaker 客户端已实现并默认关闭；真实图片 URL TTL、内容安全返回、默认封面兜底策略、失败码、计费口径和联调窗口仍待确认。
- Suno/MiniMax 真实音乐已补充一键 stack smoke，默认先跑 `suno` 单作品，可自动启动 worker/API、调用单作品 smoke 并清理进程；当前仍需用户明确执行真实调用窗口后再产生真实 Provider 样本。
...
2. 若用户要测真实 AI 出歌，优先用 `ALLOW_DREAMMAKER_REAL_SMOKE=1 REAL_PROVIDER=suno scripts/smoke/dreammaker-real-music-stack-smoke.sh` 执行单作品 Suno stack smoke；如已手动启动 worker/API，则按 `docs/runbook/dreammaker-controlled-real-integration.md` 使用单作品真实音乐 smoke。不得在默认 `sync` 模式或无成本止损时运行。
3. DeepSeek 真实写词联调只需用户安全注入 `DEEPSEEK_API_KEY` 并确认样本数量；按 `docs/runbook/deepseek-controlled-real-integration.md` 先跑 1 条灵感成歌样本。
4. Image2 真实封面联调需安全注入 DreamMaker AK/SK，保持 `MUSIC_PROVIDER=mock`，按 `docs/runbook/image2-controlled-real-integration.md` 先跑 1 条确认出歌样本。
