...
- Image 2 DreamMaker 生产目标客户端与 WellAPI 当前公网客户端均已实现并默认关闭；WellAPI Image2 已补一键 stack smoke 和低层单作品 smoke，但尚未执行真实图片调用。真实图片 URL TTL、内容安全返回、默认封面兜底策略、失败码、计费口径和联调窗口仍待确认。
...
2. 若用户要测真实 AI 出歌，当前公网优先用 `ALLOW_YUNWU_REAL_SMOKE=1 scripts/smoke/yunwu-suno-real-music-stack-smoke.sh` 执行单作品 Suno stack smoke；进入公司内网或生产目标路径时，再用 `ALLOW_DREAMMAKER_REAL_SMOKE=1 REAL_PROVIDER=suno scripts/smoke/dreammaker-real-music-stack-smoke.sh` 验证 DreamMaker。不得在默认 `sync` 模式或无成本止损时运行真实音乐。
3. DeepSeek 真实写词联调只需用户安全注入 `DEEPSEEK_API_KEY` 并确认样本数量；按 `docs/runbook/deepseek-controlled-real-integration.md` 先跑 1 条灵感成歌样本。
4. Image2 真实封面联调当前公网可安全注入 `WELLAPI_API_KEY`，保持 `MUSIC_PROVIDER=mock`，运行 `ALLOW_WELLAPI_IMAGE2_REAL_SMOKE=1 scripts/smoke/wellapi-image2-real-cover-stack-smoke.sh` 先跑 1 条确认出歌样本；生产目标 DreamMaker Image2 路径仍按 `docs/runbook/image2-controlled-real-integration.md` 切换 `IMAGE2_BACKEND=dreammaker` 后验证。
