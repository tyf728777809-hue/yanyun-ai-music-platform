...
更新时间：2026-06-18 00:45 CST

## 当前阶段

项目已完成第 8 批 MinIO/S3 发布包强化，并已按用户要求在前端并行开发期间推进第 10 批公司 Adapter 接入与部署交接准备和第 11 批 render-worker 本地进程调用边界：数据库 migration、Work 领域状态机、Mock Adapter 边界、OpenAPI v0.1 主路径 API、本地 Mock 作曲与发布包、DreamMaker / Suno / MiniMax 受控真实调用实现、Outbox v0.1、API outbox 到独立 Temporal worker 的编排边界、真实音乐 Provider 运行时硬保护、DeepSeek / Prompt / Lyrics 写词边界、OpenAI 兼容 `RealDeepSeekLyricsClient`、受控燕云创作知识库边界、商用分域知识库基线、CoverGenerationService / VideoRenderService 媒体生成边界、DreamMaker Image2 与 WellAPI Image2 封面真实客户端、render-worker 本地 16:9 MP4 样例渲染、Java 可配置调用 render-worker CLI 的本地进程模式、发布包 JSON 的 local / S3-MinIO 可切换对象存储，以及内部公司接入 readiness 报告均已落地。2026-06-13 用户确认不做联网搜索，但需要燕云创作知识库补足公测后角色、剧情、地域、玩法和玩家语境；项目口径已从“完全取消知识库”更新为“受控、版本化、官方资料优先、实体优先 + pgvector 辅助的创作知识库”，默认仍可 disabled。当前商用入口为 `knowledge-base/commercial-final/`，包含 280 个实体、917 个 chunk 和 339 条 eval case，缺口盘点表已全部回写为 `present`，P5 离线证据包已生成；剩余重点是真实模型 A/B 人工评分、公司资料覆盖、品牌禁区校准和 pgvector 真实延迟/召回验证。2026-06-07 已执行真实 DeepSeek、Yunwu Suno、WellAPI Image2 单项 smoke，并完成公网真实完整体验样本；2026-06-11 又用 DeepSeek v4Pro + Yunwu Suno `chirp-fenix` / v5.5 + WellAPI Image2 + local-process MP4 跑通作品 `20fa149f-d5b1-40b2-9b22-8561148158db` 到 `GENERATED / PACKAGE_READY`，MP4 已验证 H.264 video + AAC audio 双轨。Yunwu timestamped lyrics 已用同一 `chirp-fenix` 样本验证，但当前供应商 base URL 未暴露可用 JSON 时间轴接口：默认路径返回 404，若干 `/suno/*` 探测路径返回 HTML。DreamMaker 音乐与 DreamMaker Image2 仍保留为正式生产目标；当前公网环境下 DreamMaker Suno 曾返回 HTTP 403，用户判断可能需要公司内网或供应商权限。公司账号、审核、权益、社区发布、分享和生产部署形态仍由公司系统接入或决策；所有真实外部调用仍需显式开关和安全注入凭据。

2026-06-17 22:47 CST 已推进 P0/P1 真实测试闭环收口：本地 `main` 已推送到 `origin/main` 并同步，公网真实测试后端已用当前代码重启，readiness 显示 DeepSeek real、Yunwu Suno `chirp-fenix`、WellAPI Image2、S3ObjectStorageClient、`album-ffmpeg`、worker 写词任务和音乐/封面并行均为 `READY_FOR_LOCAL`；公网前端 `/`、前端代理 `/api/v1/me` 和媒体 tunnel health 均返回 200。执行 1 条完整真实 smoke，作品 `9ad48f13-e35d-423e-84ab-d8be2feeae23` 达到 `GENERATED / PACKAGE_READY`，Yunwu 音乐、MP4 和发布包链路成功；但复查数据库发现该样本封面为 `default-cover` fallback，根因是 WellAPI Image2 同步请求超时，旧 smoke 只检查封面 URL 存在而没有检查 `COVER.provider`，因此会把 fallback 误判为真实 WellAPI 封面成功。已修复真实验收脚本：`public-real-full-experience-stack.sh` 默认要求 `COVER.provider=wellapi-image2` 且 `fallback=false`，否则失败；只有显式设置 `ALLOW_COVER_FALLBACK_IN_PUBLIC_REAL_SMOKE=true` 才允许兜底封面通过。同步把 public real / WellAPI smoke 的 `WELLAPI_REQUEST_TIMEOUT` 默认提升到 `300s`，`.env.example` 与规格文档已更新。Yunwu 漏单复查显示最新样本正常，但历史仍有少量“供应商成功、平台未完整收尾”的旧作品：其中部分可通过音频导入、封面、视频或发布包恢复链路续跑，不应重新调用 Suno。润色耗时拆解样本 `cf69ecce-e631-4b0f-86d9-abb0be0eeec0` 显示首轮写词约 `162s`，其中 `CreativeBriefAgent` 等待约 `89s` 后因 JSON 无效失败并降级，`LyricsAgent` 约 `26s`，strict 质量门约 `43s`；AI 润色约 `37s`，说明润色本身已是轻量路径，主要风险在 DeepSeek 响应波动和首轮 CreativeBrief 结构化输出稳定性。验证通过：脚本 `bash -n`、`git diff --check`、`scripts/smoke/real-model-safety-gates-audit.sh`、`OPENAPI_CONTRACT_STATIC_ONLY=true scripts/smoke/openapi-contract.sh`、`STRICT_COMMERCIAL=1` 知识库校验；知识库仍有 8 条 canonical alias warning，属既有内容质量警告，不阻断本次公网测试。剩余 P1：补 CreativeBrief JSON 输出失败的快速降级/重试策略，完整跑 6 条 `strict` vs `self-score-only` 文本对照，整理历史可恢复作品队列，必要时对 WellAPI 超时继续评估异步任务型接入。

2026-06-17 23:29 CST 已完成首轮写词链路重构 v0.12 的工程落地，目标是收敛“首轮链路太重、CreativeBrief 坏 JSON/空内容导致 90 秒级等待”的问题。本轮新增 `LYRICS_FIRST_DRAFT_MODE=full-brief|direct|conditional-brief`，API/worker 默认切到 `conditional-brief`，清晰输入走 `KnowledgeRetrieve -> LyricsAgent -> self-score/local safety`，只有短、散、只点名实体但缺少情绪角度的 `INSPIRATION` 才启用瘦身 CreativeBrief；`LYRICS` 填词、`POLISH`、`CONTINUE` 不被 CreativeBrief 拖慢。CreativeBrief 已升级为 v0.12 轻量版本，只输出 `domain_decision`、`song_core`、`singer_voice`、`central_tension`、`chorus_job`、`avoid_direction`、`knowledge_use_hint`，调用策略改为 20 秒、1 次语义尝试、900 tokens，失败直接 fallback；LyricsAgent 语义重试预算收敛为 2 次。默认质量门配置同步为 `LYRICS_QUALITY_GATE_MODE=self-score-only`，strict QualityEvaluation 保留给后台抽检/高级评审/后续 A/B，不再默认阻塞公网首轮写词。public real / deepseek stack smoke 已注入新配置，真实完整 smoke 的脱敏证据不再强制要求 `CreativeBriefAgent` 出现。验证通过：`JAVA_HOME=/opt/homebrew/opt/openjdk@21 ./gradlew --no-daemon :modules:deepseek:test :modules:lyrics:test :modules:production:test :apps:music-api:test :apps:music-worker:test spotlessCheck`、`OPENAPI_CONTRACT_STATIC_ONLY=true scripts/smoke/openapi-contract.sh`、`scripts/smoke/real-model-safety-gates-audit.sh`、`STRICT_COMMERCIAL=1 python3 scripts/knowledge/validate-yanyun-knowledge.py knowledge-base/commercial-final`、`git diff --check`。本轮未调用真实 DeepSeek/Yunwu/WellAPI/DreamMaker，未跑 12 条真实 A/B；下一步应在用户确认成本后用 12 条文本样本比较 `full-brief`、`direct`、`conditional-brief` 的耗时和人工歌词质量，再最终决定公网默认采用 B 还是 C。

2026-06-18 00:08 CST 已继续完成 v0.12 首轮写词三链路真实文本 A/B。评测脚本 `scripts/knowledge/run-yanyun-lyrics-agent-v08-ab.py` 新增 `LYRICS_AGENT_AB_PROFILE=v012-first-draft`，可一次对比 `A_v0.12_full_brief`、`B_v0.12_direct`、`C_v0.12_conditional`，并按生产策略模拟 CreativeBrief 20 秒/1 次尝试和 LyricsAgent 2 次语义尝试；旧 A/B 默认行为保留。使用 12 条真实玩家输入执行 DeepSeek 文本 A/B，只调用 DeepSeek，不调用 Yunwu、WellAPI、DreamMaker，不生成音乐/封面/视频。耗时结论：full-brief total P50 约 `42.4s`，direct 约 `26.1s`，conditional 约 `25.7s`；full-brief 质量没有稳定压过另外两组，不应继续作为默认阻塞链路。人工严评结论：full-brief 胜出 `3/12`，direct 胜出 `5/12`，conditional 胜出 `4/12`，总分 conditional 略高；尤其“弱水岸”这类短、剧情实体、氛围不明输入，conditional 触发 brief 后明显追回质量。当前决策：公网真实测试默认保持 `LYRICS_FIRST_DRAFT_MODE=conditional-brief` 与 `LYRICS_QUALITY_GATE_MODE=self-score-only`，不切回 full-brief，也不直接切纯 direct。脱敏评审摘要已保存为 `knowledge-base/lyrics-agent-v0.12-first-draft-review-summary.json`，完整 Prompt 与完整歌词仍只在 ignored 的 `build/reports/lyrics-agent/`。验证通过：脚本 `py_compile`、summary JSON 格式校验、v0.12 profile `MODE=plan/preflight`、`git diff --check`。下一步建议重启公网真实测试 API/worker 让 v0.12 默认链路生效，并用浏览器跑 1-2 次真实首轮写词确认用户侧等待体感。

2026-06-17 04:25 CST 已开始推进写歌词 Agent v0.9.2 强原句保护：基于 v0.9.1 A/B 结论，核心问题不是题材规则不足，而是 CraftPlan 会把用户已经很强的原句替换成更电影感但更不适合唱的新装置。v0.9.2 不新增默认调用次数、不改知识库、不改前端，只修 CraftPlan 第一判断：先识别用户原句是否已经可唱、可重复、可变义、能代表整首歌；如果成立，`selected_device` 必须保留原句或近似原句；只有原句太抽象、太散、太长或不可唱时，才允许另选小动作/物件/声音；另选 device 必须增强用户歌核，不能替换用户歌核。当前已同步 `RealDeepSeekLyricsCraftPlanner`、`RealDeepSeekLyricsClient`、`DefaultLyricsGenerationService`、`MockLyricsCraftPlanner`、A/B 脚本和相关测试的 `v0.9.2` / `lyrics.craft.plan.v9.2` / template version `11` 标识。待验证：Java 目标测试、spotless、脚本 plan/preflight，并优先复跑 v0.9.1 输掉的 4 条样本，至少追回 2 条才恢复 6 条完整回归。

2026-06-17 04:05 CST 按用户要求推进写歌词 Agent v0.9.1 通用判断力收束并完成 6 条真实 DeepSeek 文本 A/B。工程侧：不再为寒香寻、九流门、失败者尊严、反救世主等单个题材继续堆专属规则，而是把 CraftPlan 抽象成通用作词判断：用户强表达优先、具体胜过正确、矛盾胜过顺滑、副歌必须有功能、知识库服务歌曲、曲风不改变世界、粗粝不等于粗口。`RealDeepSeekLyricsCraftPlanner` 已升级到 `v0.9.1` / `lyrics.craft.plan.v9.1` / template version `10`，`LyricsAgent` 继续兑现 `selected_device + selected_angle + chorus_mechanism`，但移除了“对某些题材如何写”的硬编码句式；`MockLyricsCraftPlanner`、`DefaultLyricsGenerationService` 和相关测试同步升级。A/B 脚本默认当前版本改为 `B_v0.9.1`，支持 `LYRICS_AGENT_AB_CASES_PATH` 指定样本集，并在 ignored 的本地报告中输出 `latency_summary`。新增通用压力测试集 `knowledge-base/lyrics-agent-v0.9.1-eval-cases.json`，覆盖用户强短语、失败者尊严、反宏大守护、轻快但难受、开放曲风边界、粗粝但不低俗、普通人不英雄化、旧铃声等 12 条真实玩家输入；代码快照已提交为 `3b0a9f44 feat: refine lyrics craft planner v0.9.1`。真实 A/B 结果：v0.9.1 只明显胜出 `2/6`，v0.7 胜出 `4/6`，未达到扩大到 12 条的门槛；v0.9.1 在“开放曲风但不现代漂移”和“粗粝但不低俗”更稳，但在“还是会出手”“不是废物”等强用户原句题上仍会替换成新的声音装置，导致更设计化但不一定更有用户心跳。耗时方面，A 组 total P50 约 `64.7s`，B 组 total P50 约 `95.5s`，CraftPlan 自身 P50 约 `26.9s`，因此在质量不明显胜出前，不建议把 v0.9.1 当作最终默认。脱敏评审摘要已保存为 `knowledge-base/lyrics-agent-v0.9.1-full6-review-summary.json`；完整 Prompt 和完整歌词只保存在 ignored 的 `build/reports/lyrics-agent/`。验证通过：脚本 `py_compile`、v0.9.1 样本 JSON 格式、A/B plan/preflight/execute、Java `:modules:creative-agent:test :modules:deepseek:test :modules:lyrics:test`、相关模块 `spotlessCheck` 和 `git diff --check`。本轮只调用 DeepSeek 文本，不调用 Yunwu、WellAPI、DreamMaker，未生成音乐、封面或视频。下一步建议只调 CraftPlan 的第一判断：用户原句是否已经是最好的 hook / device；如果是，必须保留原句或近似原句，而不是另造更电影感的装置。

2026-06-17 02:30 CST 已完成写歌词 Agent v0.9 首轮实现与真实 DeepSeek 文本 A/B：新增 `LyricsCraftPlan` 轻量候选选择层，只在 `INSPIRATION` 首轮写词前运行，输出 `song_thesis_guard`、`selected_device`、`selected_angle`、`chorus_mechanism`、`yanyun_boundary_guard` 和 `rejected_alternatives`，最终 LyricsAgent 只围绕选中的声音装置、进入角度和副歌机制写词；`POLISH`、`CONTINUE` 和 `LYRICS` 填词成歌不调用 CraftPlan，避免润色/续写变慢和用户已有歌词被改坏。API / worker 已新增 `LYRICS_CRAFT_PLANNER_ENABLED=true` 配置，真实 DeepSeek 模式使用 `RealDeepSeekLyricsCraftPlanner`，Mock/测试路径可降级；CraftPlan 失败会记录 fallback 并回到 v0.8.4 直写，不阻断用户创作。6 条小样本结果为 v0.9 胜出 4/6，12 条中样本结果约为 9/12，完整 24 条评测约为 18/24，达到进入默认首轮灵感写词方向的门槛；脱敏摘要分别保存为 `knowledge-base/lyrics-agent-v0.9-full6-review-summary.json`、`knowledge-base/lyrics-agent-v0.9-mid12-review-summary.json`、`knowledge-base/lyrics-agent-v0.9-full24-review-summary.json`，完整 Prompt 和歌词仅保存在 ignored 的 `build/reports/lyrics-agent/`。当前判断：v0.9 比继续堆 Prompt 规则更稳定，能提升“听得懂、记得住、像一首歌”的概率，尤其在清河、新手村、不羡仙、九流门、偷师、普通玩家故事和小意象题材上更有效；但仍不是最终金曲系统，寒香寻、反宏大守护、失败者尊严等个别题 v0.7 仍可能更锐利。首批观测 CraftPlan 会增加一次 DeepSeek 调用，耗时不可忽略，因此继续限定为首轮 `INSPIRATION`。验证通过：`python3 -m py_compile scripts/knowledge/run-yanyun-lyrics-agent-v08-ab.py`、`JAVA_HOME=/opt/homebrew/opt/openjdk@21 ./gradlew --no-daemon :modules:creative-agent:test :modules:deepseek:test :modules:lyrics:test`、`:apps:music-api:compileJava :apps:music-worker:compileJava`、相关模块 `spotlessCheck`、JSON 校验和 `git diff --check`。下一步建议：不要再向 LyricsAgent 主 Prompt 堆规则，改为继续微调 CraftPlan 的选择策略，重点包括保留用户原句作为 device、角色歌选择角色独有动作/物件、反宏大题材避免熟悉英雄语、粗粝题材的粗口策略由产品单独决定。

2026-06-15 21:00 CST 已完成“首次写词异步化”长期稳定方案的第一阶段落地：`POST /works/inspiration` 与 `POST /works/lyrics` 不再在 HTTP 请求内等待 DeepSeek 完成，而是立即创建 `LYRICS_GENERATING` 作品、写入 `generation_jobs` 和泛化后的 `lyrics_edit_jobs` 文本任务，前端跳转作品页后通过 `GET /works/{work_id}` 轮询到 `LYRICS_READY`。新增 migration `V202606152115__generalize_lyrics_tasks.sql`，允许 `lyrics_edit_jobs` 承载 `CREATE_INSPIRATION` / `CREATE_LYRICS` / `POLISH` / `CONTINUE`，并增加 `request_payload_json` 保存首轮写词请求摘要；`LyricsEditJobProcessor` 已支持首轮写词成功落第一版 `lyrics_draft`、失败进入 `LYRICS_FAILED`、瞬时失败按任务重试机制重排。前端和 OpenAPI smoke 已同步异步首轮语义。验证通过：`:apps:music-api:compileJava :apps:music-api:compileTestJava`、`:apps:music-api:test --tests '*WorkServiceWorkflowDispatchTest' --tests '*LyricsEditJobProcessorTest'`、Claude Web v1 `WorkPage.test.tsx`、`npm test && npm run typecheck && npm run build`、`OPENAPI_CONTRACT_STATIC_ONLY=true scripts/smoke/openapi-contract.sh`、`spotlessCheck :apps:music-api:test :modules:production:test :modules:lyrics:test :modules:deepseek:test`、`:apps:music-api:bootJar :apps:music-worker:bootJar`、`git diff --check` 和 diff 敏感信息扫描。已重启本地真实测试 API / worker，readiness 显示 DeepSeek real、Yunwu Suno real、WellAPI Image2 real、S3ObjectStorageClient、album-ffmpeg、outbox/Temporal 均为 `READY_FOR_LOCAL`；真实 DeepSeek 首轮写词 smoke 作品 `ee43fda8-6d99-4230-b67f-6fccb2d24bbf` 的创建请求 `0.09s` 返回 `LYRICS_GENERATING`，后台约 `62s` 后进入 `LYRICS_READY / WAITING_CONFIRM` 并生成歌词草稿，未触发 Yunwu/WellAPI/视频。当前阶段仍复用 API 内 `LyricsEditJobDispatcher` 承接文本任务，已解决用户侧断连与公网长 HTTP 风险；公司部署前建议第二阶段把文本任务 dispatcher 下沉到 worker 或独立 queue consumer，统一水平扩展和进程隔离。

2026-06-15 23:10 CST 已完成“长期商用收口修复 v2”核心架构收束：首轮写词、AI 润色、AI 续写的 `LyricsEditJobDispatcher` / `LyricsEditJobProcessor` 已从 `music-api` 迁入 `modules:production` 并由 `music-worker` 默认开启调度，`music-api` 默认 `LYRICS_EDIT_DISPATCHER_ENABLED=false`，只负责创建任务、返回状态和接口校验；`music-worker` 默认 `LYRICS_EDIT_DISPATCHER_ENABLED=true`，承接后台 DeepSeek 文本任务。`mood` 已贯穿 `LyricsGenerationRequest`、`CreativeBriefRequest`、DeepSeek 写词和 prompt 渲染链路，用户在首页填写的心情不再只是前端字段。确认出歌后的生产 workflow 新增 `SONG_PRODUCTION_PARALLEL_MEDIA_ENABLED=true` 与 `SONG_PRODUCTION_MEDIA_PARALLELISM=2`：音乐分支和封面分支可用受控 executor 并行执行，`MUSIC_GENERATING`、`COVER_GENERATING`、`VIDEO_RENDERING`、`PACKAGE_BUILDING` 均写入 `generation_job_steps`，视频和发布包仍等待音频与封面都成功后再执行；封面失败时可保留已生成音频并走收尾重试，不重复消耗音乐生成。`/internal/integration-readiness` 已增加 lyrics dispatcher 与 media parallelism readiness 项，真实测试脚本继续强制 S3/MinIO + `album-ffmpeg`，避免公网朋友看到不可播放的 local URL。Claude Web v1 已同步填词入口文案、后台写词轮询和非生成中页面连接恢复提示。知识库导入脚本 `DRY_RUN=1` 默认只输出摘要，不再打印完整 SQL/语料/embedding；商用知识库严格校验通过但保留少量 intentional alias collision warnings，原因是部分剧情/地点共享同名实体，当前选择不强行合并以免召回误绑。验证通过：Java `:apps:music-api:test :apps:music-worker:test :modules:production:test :modules:lyrics:test :modules:deepseek:test :modules:knowledge:test :modules:config-center:test`、`spotlessCheck :apps:music-api:bootJar :apps:music-worker:bootJar`，Claude Web v1 `npm test && npm run typecheck && npm run build`，知识库 `STRICT_COMMERCIAL=1 validate` 与 `DRY_RUN=1 import`，OpenAPI 静态契约、真实模型安全门审计、`git diff --check` 和 diff 敏感信息扫描。本轮未调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker，未生成新音乐、封面或视频；后续可在用户确认成本后跑 1 条真实完整样本，比较音乐/封面并行前后的等待时间。

2026-06-16 02:25 CST 已完成长期商用修复执行计划阶段 1：收口封面质量门误判与 `COVER_GENERATION_FAILED` 可恢复链路。用户公网实测作品 `ef0df6c5-25fb-4d77-ad4f-ba98f1f031da`《借火》失败根因定位为封面质量门将嵌套 `negative_prompt` / provider option 中的禁用词误当作正向违规文本，同时模型 reason 已明确“封面 prompt 仅包含歌名、无额外文本、符合规则”。已修复 `RealDeepSeekQualityEvaluationAgent` 的 COVER 本地安全判定：递归检查结构化 context 时跳过 negative / exclude / forbidden / avoid 类字段，允许受控歌名主标题通过，继续阻断正向假歌手、假厂牌、假版权、水印、二维码、乱码和 UI。状态与恢复链路同步完成：`COVER_GENERATION_FAILED` 下发真实 `RETRY_COVER`，`cover/regenerate` 可在已有 AUDIO 的失败作品上 reserve 后以 `reuseExistingAudio=true` 重新进入封面、视频和发布包收尾，不重复调用 Suno；Workflow 对封面阶段失败标记为可重试。已对《借火》执行受控收尾重试，最终达到 `GENERATED / PACKAGE_READY`，`media_assets` 中 audio、cover、video、timeline 齐全，MP4 为 1920x1080 且含 audio/video 双轨；`provider_calls` 仅保留原始一次 `SUNO / MUSIC_GENERATION / yunwu:suno:chirp-fenix / SUCCEEDED`，确认没有重复消耗音乐生成。本阶段验证通过：DeepSeek cover quality gate 目标测试、WorkStateMachine cover retry 测试、WorkService cover retry 测试、`spotlessCheck :apps:music-api:bootJar :apps:music-worker:bootJar`、`git diff --check`。当前未处理公网访问控制/限额；下一阶段继续修长任务稳定性、状态闪屏和真实测试 readiness。

2026-06-16 12:58 CST 已完成 P0 公网真实用户测试启动与验证收口：排查发现此前“生成歌词长时间卡住”的直接原因不是 DeepSeek 无响应，而是本地真实测试环境使用临时 `screen + bootRun` / `nohup` 启动方式不可靠，出现 API 仍在线但 `music-worker` 已退出的假健康状态，导致 `lyrics_edit_jobs` 停在 `QUEUED` 或 `RUNNING` 等待锁过期。新增 `scripts/smoke/public-real-test-stack.sh` 作为小范围公网真实测试后端启动入口：从 `.env.real.local` 或当前 shell 读取凭据，强制 DeepSeek real + Yunwu Suno + WellAPI Image2 + S3/MinIO + `album-ffmpeg` + 音乐/封面并行配置，默认用独立 `screen` session 承载 API/worker，并检查 API health、worker health 与 `/internal/integration-readiness`；`status` 现在同时检查 API/worker 进程和健康接口，任一服务掉线都会返回失败，不再把半活状态误判为可测试；公网测试脚本中 `LYRICS_EDIT_LOCK_TIMEOUT` 默认降为 `3m`，worker 意外退出时能更快被新 worker 回收。用该脚本重启后，卡住作品 `17859f83-dede-4ea8-8e29-680a61163f8e` 被新 worker 回收并推进到 `LYRICS_READY / WAITING_CONFIRM`。随后用公网前端完成 1 条真实浏览器自动化样本 `0d8e938f-2a40-423e-9a33-d2920c550868`：首轮写词约 `128s`，AI 润色约 `137s`，确认出歌后音乐 `91.3s`、封面约 `192s` 且时间重叠，视频约 `7.3s`，发布包约 `8.1s`，最终达到 `GENERATED / PACKAGE_FETCHED`；浏览器无非预期 HTTP 4xx/5xx 和 console error，390px/1440px 无横向溢出，发布包 audio/cover/video/timeline 公网 Range GET 均为 `206`，MP4 经 ffprobe 验证为 H.264 1920x1080 + AAC 双轨、时长约 `134.28s`。12:58 复查并显式重启公网真实测试后端，当前 API 与 worker 均为 alive/health yes，readiness 显示 DeepSeek real、Yunwu、WellAPI、S3、album-ffmpeg 和音乐/封面并行均为 `READY_FOR_LOCAL`。当前公网前端与媒体 tunnel 仍适合可信小范围测试；残留风险仍是用户已明确暂缓的公网访问控制/次数限额，以及 DeepSeek 写词/润色真实耗时偏长。

2026-06-16 13:06 CST 继续修复 P0 公网真实测试栈误伤问题：复查发现 `scripts/smoke/real-model-safety-gates-audit.sh` 在验证 `public-real-full-experience-stack.sh` gate 失败路径时，会触发该旧脚本的 `trap cleanup`，而旧 cleanup 在未启动自身 API/worker 的情况下仍无差别清理 8080/8081 端口 listener，导致当前正在服务公网测试的 `music-worker` 被误杀。已修复 `public-real-full-experience-stack.sh`：仅在脚本通过端口检查并准备启动自身 stack 后才允许 cleanup 清理端口 listener；gate/preflight 阶段失败只清理自己持有的 PID，不再扫射外部运行环境。验证通过：重启 `public-real-test-stack` 后再次运行 `scripts/smoke/real-model-safety-gates-audit.sh`、`OPENAPI_CONTRACT_STATIC_ONLY=true scripts/smoke/openapi-contract.sh`、`git diff --check`，随后 `scripts/smoke/public-real-test-stack.sh status` 仍显示 API/worker 均 alive/health yes。

2026-06-16 15:21 CST 已修复公网真实出歌阶段“作品收尾时出了点问题”的长期恢复链路。最新失败作品根因定位为：Yunwu Suno 音乐生成任务已成功并返回 provider task id，但后端下载供应商音频 URL 入对象存储时遇到远程 HTTP 403，旧逻辑把该问题归类成泛化 `PACKAGE_BUILD_FAILED`，并且 `MUSIC_GENERATING` 在音频真正入库前就被标记成功，导致前端只能提示联系客服，无法只恢复音频导入。已新增 `RemoteObjectImportException` 保存远程状态码和可重试语义，`HttpRemoteObjectImporter` 会把 401/403/404/429/5xx 等远程下载失败明确交给生产 workflow；`MusicProvider` 增加 `refreshAudio` 边界，Yunwu 实现通过已有 task id 调用 fetch/status 刷新音频地址，不重新提交 Suno 出歌任务。生产 workflow 现在只有在 AUDIO 成功入库后才标记 `MUSIC_GENERATING` 成功；若音频导入失败，会先尝试刷新音频 URL 并重导入一次，仍失败则进入 `AUDIO_IMPORT_FAILED` 并下发真实 `RETRY_AUDIO_IMPORT`。API 新增 `POST /api/v1/works/{work_id}/music/audio-import/retry`，会复用最新成功 `provider_trace_id` 和已生成封面，只重新获取音频、补视频和发布包，不重复消耗音乐生成；Claude Web v1 失败页已增加“重新获取音频”动作和友好文案。并行封面分支也增加 job-running 防护，避免主 workflow 已失败后异步封面继续写入新资产。验证通过：Java `:apps:music-api:test :modules:production:test :modules:storage:test :modules:suno:test :modules:work-domain:test`、`spotlessCheck`，Claude Web v1 `npm test && npm run typecheck && npm run build`，OpenAPI 静态契约、`git diff --check` 和 diff 敏感信息粗扫。当前未调用真实 DeepSeek/Yunwu/WellAPI/DreamMaker，未对失败作品直接改库假装成功；下一步如需恢复旧失败作品，应在重启 API/worker 后用新的 `RETRY_AUDIO_IMPORT` 路径执行受控恢复。

2026-06-14 22:36 CST 已完成通用燕云实体纠错与角色/剧情接地修复：新增 `KnowledgeEntityResolver`、`KnowledgeAliasCandidate`、`ResolvedKnowledgeEntity` 与 match kind，`JdbcKnowledgeService` 现在先做实体直接命中，再用轻量字符级纠错补高置信错字/俗称，并把 resolved entities 随 `KnowledgeRetrievalResult` 传给写词与质量门；短词和歧义 fuzzy 命中不会进入检索。商用知识库补充了 `姜无浪/姜晏/姜叔`、`若水岸寻心`、`若水岸/弱水之岸` 等高频别名，并新增 `姜无浪 -> 江无浪`、`若水岸 -> 弱水岸`、单字“寻”不强召回的 eval case。`LyricsAgent` prompt 已强化：点名角色/剧情/地点/门派时必须按 canonical entity 使用知识库核心材料，不把用户错字当正式名；`QualityEvaluationAgent` 已强化点名实体一致性检查，并增加本地兜底：若 context 已显示 fuzzy 纠错但歌词仍输出错字名，则直接 `REWRITE`。验证通过：`STRICT_COMMERCIAL=1 python3 scripts/knowledge/validate-yanyun-knowledge.py knowledge-base/commercial-final`、`DRY_RUN=1 python3 scripts/knowledge/import-yanyun-knowledge.py knowledge-base/commercial-final`、`JAVA_HOME=/opt/homebrew/opt/openjdk@21 ./gradlew --no-daemon :modules:knowledge:test :modules:lyrics:test :modules:deepseek:test`、`JAVA_HOME=/opt/homebrew/opt/openjdk@21 ./gradlew --no-daemon spotlessCheck`、`git diff --check`、`:apps:music-api:bootJar :apps:music-worker:bootJar` 均通过。已把新知识库实际导入本地 PostgreSQL，并重启本地真实测试 API / worker：`http://localhost:8080/health` 和 `http://localhost:8081/actuator/health` 均为 OK，前端 `http://127.0.0.1:5275/` 可继续真实测试。未调用真实 DeepSeek/Yunwu/WellAPI/DreamMaker 生成新作品；当前 PostgreSQL collation version warning 仍是既有本地环境提示，不影响本轮检索验证。

2026-06-13 19:15 CST 已按“燕云创作知识库二阶段方案 v0.3”完成首批工程落地：ADR 0005 已从“取消独立知识库”更新为“不做开放联网/RAG，建设受控、版本化、官方资料优先的燕云创作知识库”；新增规格 `docs/specs/yanyun-creative-knowledge-base-v0.3.md`、种子资料 `knowledge-base/yanyun-official-kb-v1.json`、pgvector migration `V202606131900__add_yanyun_creative_knowledge_pgvector.sql` 和导入脚本 `scripts/knowledge/import-yanyun-knowledge.py`。本地 PostgreSQL compose 镜像切为 `pgvector/pgvector:pg16`，`KnowledgeService` 新增 `KNOWLEDGE_RETRIEVAL_MODE=disabled|mock|pgvector`，默认 disabled；pgvector 模式实体别名优先、语义检索辅助，检索失败降级为空知识上下文，不阻断创作。写词链路新增 `KnowledgeRetrieve` 审计记录，只保存 `kb_version` / chunk id hash 等摘要；`LyricsAgent`、`CreativeBriefAgent`、`QualityEvaluationAgent` 口径已从“必须出现燕云关键词”改为“燕云大世界归属 + 用户故事保真 + 不混入其他 IP”，普通玩家故事不强绑官方角色。首版种子只用于工程打通和召回验证，覆盖寒香寻/寻心、清河/开封/雁门、不羡仙、小人物侠义、门派倾覆、奇术、偷师、寻声等样例；仍需公司/官方资料包和游戏内实录继续补齐完整角色、剧情/任务线、地域、势力/门派和玩法体验。验证通过：导入脚本 `DRY_RUN=1` 可生成 SQL，`python3 -m py_compile scripts/knowledge/import-yanyun-knowledge.py` 通过，`git diff --check` 通过，`JAVA_HOME=/opt/homebrew/opt/openjdk@21 ./gradlew --no-daemon :apps:music-api:compileJava :apps:music-api:processResources :modules:knowledge:test :modules:lyrics:test :modules:deepseek:test --tests '*DefaultLyricsGenerationServiceTest' --tests '*RealDeepSeekCreativeAgentsTest' --tests '*DeterministicKnowledgeEmbeddingServiceTest' --tests '*KnowledgeRetrievalModeTest' spotlessCheck` 通过。当前未重建本地 PostgreSQL 容器、未执行真实 pgvector migration/导入、未调用真实 DeepSeek 或外部 embedding API。

2026-06-13 19:25 CST 已继续把燕云创作知识库接入到本地真实数据库验证闭环：本地 `yanyun-postgres` 容器已从旧 `postgres:16` 重建为 `pgvector/pgvector:pg16`，保留原有 volume 数据；重建 API / worker bootJar 后启动 API，Flyway 成功执行 `V202606131900__add_yanyun_creative_knowledge_pgvector.sql`，数据库已有 `vector` 扩展、知识库表和 Flyway 成功记录。`scripts/knowledge/import-yanyun-knowledge.py` 已补本机无 `psql` 时的 Docker fallback，当前环境成功导入 `yanyun-official-kb-2026-06-13-v1`：`knowledge_entities=22`、`knowledge_entity_aliases=81`、`knowledge_chunks=21`，寒香寻、九流门、玉露为乡、中式解谜、武学自由搭配等别名均可命中。以 `KNOWLEDGE_RETRIEVAL_MODE=pgvector`、真实模型关闭、Mock 写词模式创建测试作品 `a99ba7fa-ab5f-4f31-8f5c-02662196f25b`，`agent_runs` 记录 `KnowledgeRetrieve / v0.3 / SUCCEEDED / latency_ms=36`，`lyrics_drafts.knowledge_base_version=yanyun-official-kb-2026-06-13-v1`，`yanyun_references_json=["寒香寻","寻声","中式解谜","开封","开封夜色"]`。本轮未调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker 或外部 embedding API。当前本地库因镜像切换出现 PostgreSQL collation version warning，不影响本轮检索验证；后续可单独决定是否重建/刷新本地数据库 collation。
...
| 2026-06-12 05:26 CST | 启动公司同事公网临时试用环境 | 基于最新 `main` 启动本机公网临时试用：Docker 基础设施继续使用本地 PostgreSQL/Redis/Temporal/MinIO，`music-api` 与 `music-worker` 使用真实 DeepSeek v4Pro、Yunwu Suno `chirp-fenix`、WellAPI Image2、`album-ffmpeg` 默认视频，公司账号/审核/权益/发布/分享仍为 Mock Adapter；前端使用 Claude Web v1，媒体文件通过 MinIO + Cloudflare 临时 tunnel 暴露。已验证 API health、worker health、公网前端 200、公网前端代理只读 API 200、媒体 tunnel health 200，未在验证过程中发起新的真实生成任务。公网地址为临时运行态信息，只在最终回复中给出，不写入仓库文档 |
| 2026-06-12 05:38 CST | 取消公网试用 20 次出歌上限 | 按用户明确要求取消刚加入的 20 次真实出歌锁定上限：`MockQuotaAdapter` 恢复为本地 mock 权益无限锁定行为，API/worker 不再读取 `yanyun.quota.mock-max-generate-locks` 配置，公网运行环境重启时不再设置 `YANYUN_QUOTA_MOCK_MAX_GENERATE_LOCKS`。保留原有安全边界：真实模型仍只在当前运行环境凭据下调用，公司账号/审核/权益/发布/分享仍是 Mock Adapter，DreamMaker 仍是正式生产目标 |
| 2026-06-12 13:42 CST | 修复真实收尾阶段 DeepSeek JSON 截断风险 | 用户实测作品收尾失败的直接原因已定位：Yunwu Suno 音频生成和导入成功，但 `CoverPromptAgent` 调 DeepSeek 生成封面 prompt 时返回非法/不完整 JSON，导致后续封面、视频和发布包未继续。已将 DeepSeek Agent JSON 抽取兜底集中到 `DeepSeekAgentJson`，歌词客户端也支持 fenced/wrapped JSON；MusicPrompt/CoverPrompt/Quality Agent 不再硬编码低 `max_tokens`，统一尊重 `DEEPSEEK_RESPONSE_MAX_TOKENS`，默认从 1800 提高到 4096；CoverPromptAgent 改为只传歌词摘录和必要上下文，降低截断概率；相关脚本、应用配置、`.env.example` 和 DeepSeek runbook 已同步。验证通过：`:modules:deepseek:test`、`spotlessCheck`、`:apps:music-api:compileJava`、`:apps:music-worker:compileJava`。公网临时试用 API/worker 已重启并确认加载 4096 token 配置，公开前端代理只读 API 正常。剩余缺口：已失败的旧作品当前没有“从已生成音频继续补封面/视频/发布包”的 UI/接口恢复能力，只能返回编辑或新建作品；后续需设计可恢复的收尾重试接口 |
| 2026-06-12 14:16 CST | 完成旧失败作品手动收尾恢复 | 按用户要求将旧失败作品 `8dc711a7-bf02-41d3-bba1-1b5ee7bd773f` 补齐到可前端查看状态：未重新调用 Suno 出歌，复用已入库音频，补生成封面、album-ffmpeg 默认视频、无字幕 timeline 和发布包，并将作品状态更新为 `GENERATED / PACKAGE_READY`。验证通过：本地与公网 API 均返回 `GENERATED`、`PACKAGE_READY`，`media_assets` 中 audio/cover/video 均存在；cover/video 通过 Cloudflare 临时媒体 tunnel Range 请求返回 HTTP 206；MP4 已由恢复脚本校验含 video/audio 双轨。临时恢复脚本和中间产物位于 `build/`，不进入 Git。后续仍需补正式“收尾阶段恢复/续跑”接口，避免再靠一次性脚本处理旧失败作品 |
| 2026-06-12 14:55 CST | 修复 DeepSeek 空内容导致的出歌前置失败 | 用户实测作品 `409a8bb8-8ecc-4b76-b3c3-60059b6a59b1` 显示“旋律生成遇到了问题”，排查确认并未调用到 Suno/Yunwu，失败点是 `MusicPromptAgent` 调 DeepSeek 生成音乐 prompt 时收到空内容，Workflow 将其归类为 `MUSIC_GENERATION_FAILED` 且作品可重试。已在 `DeepSeekJsonChatClient` 增加 200 响应但 content 为空/不可解析时的单次语义重试，并补充本地假 HTTP 回归测试；验证通过：`:modules:deepseek:test`、`spotlessCheck`、`:apps:music-api:compileJava`、`:apps:music-worker:compileJava`、`bootJar`。公网临时试用环境已改为 `java -jar` 启动 API/worker，避免双 `bootRun` 抢 Gradle 缓存锁；本地 API/worker health 与公网业务只读代理均正常。当前失败作品仍保持可重试状态，用户可点击“重新生成”继续真实出歌 |
| 2026-06-12 16:36 CST | 执行真实用户模拟完整测试并暂停用户实测放行 | 已用 Playwright/公网 API/ffprobe 模拟真实用户走查：Claude Web v1 `?mock=1` 全功能分支通过，覆盖灵感成歌、填词成歌、润色/续写、第三次 409、失败重试、作品列表、404、成品交接、刷新链接、标记已交接和 390px/1440px 布局；历史真实样本 `58dc1a8b-5435-43ee-a553-16bf5c6dc361` 已完成 `GENERATED / PACKAGE_FETCHED`，浏览器确认封面存在、音频/视频 metadata 可读且时长约 210.48s，MP4 为 H.264 1920x1080 + AAC 双轨，发布包刷新与交接可用。测试中发现新问题：图片模型直接生成封面文字会写错标题，已将真实 `CoverPromptAgent` 运行策略改为 `NO_TEXT_IN_IMAGE`，并同步 `docs/specs/yanyun-creative-agent-v0.5.md` 与 `docs/specs/default-video-v5-cover-quality-gate.md`；`DefaultLyricsGenerationService` 已改为用 `CreativeBriefAgent` 的 `yanyun_references` 落库，避免取消知识库后前端“燕云意象”为空。随后连续真实样本 `6079e488-03d4-4426-ab63-b1af37912d9e`、`29537c1c-2cc9-439e-8c3d-f229ec292655`、`61fdd329-f8bc-4bf0-a43a-3bcbb33b5edb` 均成功完成 DeepSeek 写词与 Yunwu 音乐，但在封面质量门阶段被误判为 `PACKAGE_BUILD_FAILED`；已继续修复无字封面质量门，覆盖英文/中文 no-text 原因和非标准 decision 的本地放行规则。验证通过：`:modules:lyrics:test`、`:modules:deepseek:test`、`spotlessCheck`、`:apps:music-api:bootJar`、`:apps:music-worker:bootJar`，公网 API/worker 已重启并健康，公网 API 代理正常。当前结论：本轮没有再次消耗真实出歌额度复跑到 `PACKAGE_READY`，因此暂不建议放行用户做完整真实出歌测试；下一步只需再跑 1 条真实样本验证封面质量门修复，若到 `GENERATED / PACKAGE_READY` 再给用户测试链接 |
| 2026-06-12 17:02 CST | 真实样本复跑受供应商账户限制阻断 | 按“再跑 1 条真实样本”推进，作品 `bcc8543c-def8-441b-8f31-2e42d4223bd6` 成功完成 DeepSeek 写词，`CreativeBriefAgent` 已落库 5 条燕云引用，但确认出歌后 Yunwu 音乐阶段第一次返回账号额度/积分不足类错误，自动重试后返回供应商并发/账号上限类错误，作品最终停在 `FAILED / MUSIC_GENERATION_FAILED / PACKAGE_NOT_READY`。本次未进入 WellAPI 封面、album-ffmpeg 视频和发布包阶段，因此不能作为完整产品放行样本。已修复错误归类：新增 `PROVIDER_ACCOUNT_LIMIT`，Yunwu/DreamMaker 共享失败映射可识别中文/英文积分、余额、点数、账号额度和并发上限错误；该失败码不会下发 `RETRY_MUSIC`，前端展示为平台音乐服务额度或并发已达上限并建议联系平台协助，避免用户反复点重试。验证通过：`:modules:dreammaker:test`、`:modules:work-domain:test`、`:modules:suno:test`、`:apps:music-api:bootJar`、`:apps:music-worker:bootJar`、Claude Web v1 `npm test` / `typecheck` / `build`。公网 API/worker 已重启并健康，公网前端链接仍可打开；但在供应商账户恢复前，暂不建议用户继续真实出歌测试 |
| 2026-06-12 17:17 CST | 再次真实重试仍被 Yunwu 账号余额阻断 | 按用户“再试一下”再次提交 1 条真实样本 `cd8e42cf-eb0f-4006-9c79-ef7188551a6a`，DeepSeek 写词成功且 `yanyun_reference_count=5`；Yunwu 音乐第一次仍返回积分不足类错误，按当时后端动作自动重试后返回“信用不足”，作品停在 `FAILED / MUSIC_GENERATION_FAILED / PACKAGE_NOT_READY`，未进入封面、视频和发布包。排查确认“信用不足”是同一类供应商账户余额/额度问题，已补充 `PROVIDER_ACCOUNT_LIMIT` 映射关键词 `信用`，后续同类错误不会再展示为可重试的普通旋律失败。验证通过：`:modules:dreammaker:test`、`:modules:work-domain:test`、`:apps:music-api:bootJar`、`:apps:music-worker:bootJar`、Claude Web v1 `npm run typecheck`、`git diff --check`；公网 API/worker 已重启并健康，公网只读 API 探活通过。当前阻塞仍是 Yunwu 账户余额/信用/额度，需供应商账户恢复后再跑完整真实样本 |
| 2026-06-12 17:48 CST | 第三次真实重试仍未形成完整样本 | 按用户“再试一次”提交真实样本 `c7e613a2-efc6-4c38-832a-74088cb7175a`，DeepSeek 写词成功且 `yanyun_reference_count=5`；第一次 Yunwu 音乐调用仍返回积分不足类错误，因当时非 2xx HTTP 响应体未参与失败码映射，后端仍错误下发了 `RETRY_MUSIC`。本轮已修复 `YunwuSunoMusicProvider`：非 2xx 响应也会解析脱敏 JSON body 并按积分/余额/信用/账号限制映射为 `PROVIDER_ACCOUNT_LIMIT`，避免再次自动重试浪费真实调用；新增单元测试覆盖 402 + 积分不足响应。自动重试后的第二阶段未调用 Yunwu，而是在 `MusicPromptAgent` 后的质量门被 `MUSIC_QUALITY_FAILED` 阻断，原因是 DeepSeek 生成歌词被判定为泛古风武侠、缺少足够《燕云十六声》IP 绑定；这暴露出 `LyricsAgent` / 歌词质量门与音乐质量门标准不一致，需下一批修写词质量门前置拦截。验证通过：`:modules:suno:test`、`:modules:dreammaker:test`、`:modules:work-domain:test`、`:apps:music-api:bootJar`、`:apps:music-worker:bootJar`；公网 API/worker 已重启并健康。当前仍不建议用户做完整真实出歌测试：一方面 Yunwu 账户仍返回积分不足，另一方面写词质量门需修到更早发现泛古风跑题 |
| 2026-06-12 17:52 CST | Yunwu 非生成连通性检查 | 按用户要求先判断 Yunwu 是否能连上，本轮只调用查询接口并使用不存在的任务 ID，不提交新的生成任务、不产生出歌成本。结果：Yunwu 域名/TLS/JSON 响应正常，鉴权未返回 401/403，说明供应商服务和当前 key 基本可达；但此前真实提交仍返回积分/信用不足，因此当前判断为“能连上，但账户额度不足导致不能生歌”。 |
| 2026-06-12 19:04 CST | 前置修复歌词质量门与失败码 | 针对第三次真实重试暴露的“歌词质量门与音乐质量门标准不一致”问题，本轮未调用真实模型，完成前置修复：`DefaultLyricsGenerationService` 在 DeepSeek 写词后立即调用 `QualityEvaluationAgent`，若一次内部重写后仍为 `REWRITE/RETRY/BLOCK/MANUAL_REVIEW` 或低质量分，会抛出 `LyricsQualityException`，不再让泛古风歌词进入确认出歌和音乐阶段；`RealDeepSeekQualityEvaluationAgent` 增加本地燕云内容锚点兜底，模型误判 PASS 时会把缺少《燕云十六声》具体锚点的歌词/音乐 prompt 改为 `REWRITE`；新增 `LYRICS_QUALITY_FAILED` 失败码并同步后端状态机、API 错误处理、OpenAPI、前端友好文案和契约脚本，Workflow 会将由歌词跑题导致的音乐 prompt 质量失败映射为不可重试的歌词质量失败，只允许返回编辑。验证通过：`:modules:lyrics:test`、`:modules:deepseek:test`、`:modules:work-domain:test`、`:apps:music-api:test` 指定回归、`:apps:music-worker:compileJava`、`:apps:music-api:compileJava`、`spotlessCheck`、`:modules:suno:test`、`:modules:dreammaker:test`、Claude Web v1 `npm test` / `typecheck` / `build`、`bash -n scripts/smoke/openapi-contract.sh`、`git diff --check`。已重启公网试用 API/worker 并确认 API `/health`、worker `/actuator/health`、前端 5275 页面和代理 `/api/v1/me` 均正常。当前仍未复跑真实出歌样本，Yunwu 账号额度/余额问题仍是完整真实体验的外部阻塞 |
| 2026-06-12 19:55 CST | 复测 Yunwu Suno 真实生成 | `TARGET=yunwu-suno MODE=preflight` 已通过，API/worker 健康且 workflow 为 `outbox/temporal`；随后执行 1 次真实 Yunwu Suno smoke，作品 `2bc5d230-aae8-4f64-ab27-ff4b60dcbc59` 在歌词 draft 准备后成功进入 `MUSIC_GENERATING`，约 25 秒后被供应商返回账号积分/余额不足类错误，后端归类为 `FAILED / PROVIDER_ACCOUNT_LIMIT / PACKAGE_NOT_READY`，未生成 audio/cover/video。结论：云雾连接、鉴权和提交路径正常，但当前账户仍无法生歌，阻塞点是供应商账户余额/积分状态，不是本地 API/worker 或 workflow 配置。 |
| 2026-06-13 15:46 CST | 停止公网临时试用环境 | 按用户要求关闭此前给朋友访问的公网测试环境：已停止 Cloudflare 临时前端/媒体 tunnel、Claude Web v1 Vite 前端、`music-api`、`music-worker` 和相关 screen 会话；复查确认无 `cloudflared` 试用 tunnel、无 API/worker/frontend 进程，`8080/8081/5275/5274` 均未监听。Docker 基础设施 PostgreSQL/Redis/Temporal/MinIO 仍保留运行，用于保存本地测试数据；如需完全断开本机基础服务，可后续再停 Docker compose。 |
| 2026-06-13 15:51 CST | 启动本地真实测试环境 | 按用户要求恢复本机真实测试：启动 `music-api`、`music-worker` 和 Claude Web v1，全部只监听 `127.0.0.1`，不启用 Cloudflare 公网 tunnel；真实 Provider 配置为 DeepSeek v4Pro、Yunwu Suno `chirp-fenix`、WellAPI Image2、`album-ffmpeg` 默认视频，公司账号/审核/权益/发布/分享仍为 Mock Adapter，媒体 URL 使用本机 MinIO `127.0.0.1:9000`。已验证 API health、worker health、integration readiness 和前端 `/api/v1/me` 代理均正常；当前用户应使用不带 `?mock=1` 的本地前端地址进行真实测试。 |
| 2026-06-13 15:57 CST | 复测 Yunwu Suno 已恢复可生成 | 按用户要求执行 1 次 Yunwu Suno 真实生成 smoke，作品 `dcfad43c-e62b-4458-9526-357fd75918f2` 成功从 `MUSIC_GENERATING` 推进到 `COVER_GENERATING`，`provider_calls` 脱敏摘要显示 `SUNO / yunwu:suno:chirp-fenix / SUCCEEDED`，作品详情中 `media_assets.audio_url` 已存在，说明云雾音乐生成和音频导入已恢复。完整链路未到 `PACKAGE_READY`，后续失败点为 `PACKAGE_BUILD_FAILED`，原因是 `CoverPromptAgent` 输出被封面 Prompt 质量门判定 `BLOCK`，未生成封面/视频/发布包；下一步应修封面 Prompt 质量门/Agent 输出一致性，而不是继续排查 Yunwu。 |
| 2026-06-13 17:05 CST | 修复封面受控歌名策略 | 按用户确认改回“封面可以生成作品歌名，但不允许假歌手/署名/厂牌/版权/小字”的策略：新增 `docs/specs/cover-visual-policy-v0.2.md`，将 `CoverPromptAgent` 从 `NO_TEXT_IN_IMAGE` 改为 `CONTROLLED_TITLE_TEXT`，输出 `allowed_title=song_title`，并要求 Image2 只生成唯一文本元素即作品歌名；`QualityEvaluationAgent` 的 `COVER` gate 改为放行受控歌名标题，不再因为标题字本身 `BLOCK/REWRITE`，但仍阻断正向假署名、版权、厂牌、水印、UI 等风险。同步更新默认视频封面质量门和 Agent v0.5 规格，明确本阶段不接 OCR、不设人工确认位。验证通过：`:modules:deepseek:test --tests '*RealDeepSeekCreativeAgentsTest'`、受影响模块测试/编译、`spotlessCheck`、`:apps:music-api:bootJar`、`:apps:music-worker:bootJar`；已重启本机真实测试 API/worker 并确认 DeepSeek/Yunwu/WellAPI readiness 与前端代理正常。本轮未重新调用真实模型样本。 |
| 2026-06-13 19:21 CST | 扩充燕云创作知识库官方公开资料种子 | 按用户要求使用搜索辅助补齐知识库二阶段资料，但保持用户实时写词链路不联网搜索。新增 `docs/integrations/yanyun-knowledge-source-registry.md`，登记公开官方资料来源、资料优先级、自问自答采集方法、当前覆盖和资料缺口；扩充 `knowledge-base/yanyun-official-kb-v1.json`，新增凉州、长安、青州/蓬山/文津馆、玉露为乡、弱水岸寻心、九流线索、无名剑谱、中式解谜、武学自由搭配、营生与不平事、开封夜色等实体和创作 chunk；同步更新 `docs/specs/yanyun-creative-knowledge-base-v0.3.md` 与 `knowledge-base/README.md`。验证通过：JSON 格式检查、导入脚本 `DRY_RUN=1`、`py_compile`、受影响 Java 编译/测试/`spotlessCheck`、`git diff --check`。当前缺口：高频角色完整人生经历、人物关系、任务线结局、势力定论和官方禁写边界仍需公司资料包或游戏内实录补齐。 |
| 2026-06-13 21:20 CST | 建立商用分域知识库基线 | 按用户“可商用最终版”目标，将知识库从单文件工程种子升级为 `knowledge-base/commercial-final/` 分域资料包：人物、剧情/任务线、地域/场景、势力/门派、玩法体验、创作禁区和 eval cases。新增 `scripts/knowledge/yanyun_kb_loader.py`，导入脚本现在支持目录输入并自动合并分域资料；新增 `scripts/knowledge/validate-yanyun-knowledge.py`，支持 `STRICT_COMMERCIAL=1` 检查字段完整度、来源、别名冲突、chunk 长度、禁写项和商用数量阈值。商用版本 `yanyun-commercial-kb-2026-06-13-v1` 当前包含 63 个实体、76 个 chunk、30 条 eval case；默认 `KNOWLEDGE_KB_VERSION` 已切到商用版本，但 `KNOWLEDGE_RETRIEVAL_MODE` 仍默认 disabled。验证通过：`STRICT_COMMERCIAL=1 python3 scripts/knowledge/validate-yanyun-knowledge.py knowledge-base/commercial-final`、`DRY_RUN=1 python3 scripts/knowledge/import-yanyun-knowledge.py knowledge-base/commercial-final`、脚本 `py_compile`、知识库 JSON 检查、`git diff --check`。本轮未调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker 或外部 embedding API。当前仍不能宣称完整官方剧情百科：人物完整经历、支线结局、势力关系和公司品牌禁区需继续用游戏内实录/公司资料包补齐。 |
| 2026-06-13 21:55 CST | 调整知识库资料采集口径为自问自答创作叙事层 | 按用户确认，知识库不再按“官方/实录不足就保守占位”的单一路径推进；正式采用“玩家真实提问 -> 自问自答 -> 攻略/维基/剧情整理综合 -> 字段化入库”的创作资料采集方法。已将来源政策改为官方资料优先，但 `guide_wiki_synthesis` / `accepted_story_synthesis` 可进入创作叙事层；原始社区散帖仍不直接入库。寒香寻人物卡已从公开种子升级为 5 张剧情整合卡，覆盖洛神换脸者、不羡仙老板娘、少东家的寒姨、褚清泉旧情、弱水岸寻心和绣金楼/千夜旧怨；新增 `knowledge-base/self-qa-collection-v0.1.md` 作为后续补库任务板，收录子 Agent 对人物、剧情/地域、势力/玩法的缺口盘点。验证脚本已放开综合资料层但仍拒绝未经整理的原始社区资料；待验证通过后建议形成知识库阶段 commit。 |
| 2026-06-13 22:06 CST | 完成 P0 高频人物传首批补库 | 已先提交知识库自问自答方法快照 `c883fa1 feat: add self-qa yanyun knowledge synthesis workflow`。随后推进 P0 人物传：修正郑鄂不再作为抽象记录者，而改为常平使/常平一炬悲剧人物线；容鸢采用官方中文名优先并保留慕容源/Murong Yuan 别名；鬼市子从稳定人物传降为开封鬼市身份壳/场景故事线。新增江无浪、褚清泉、红线、伊刀、天不收 5 个高频人物实体和对应创作卡，补齐少东家童年、不羡仙家庭感、寒香寻旧情、旧人回潮、旧信旧友等写词素材；新增 7 条 eval case 覆盖这些实体。商用知识库当前为 68 个实体、89 个 chunk、37 条 eval case。 |
| 2026-06-13 22:54 CST | 完成并行补库第二批与运行时来源分层 | 已按用户要求用 5 个只读子 Agent 并行采集人物、剧情、地域/门派/玩法、来源边界和 eval 缺口，由主 Agent 统一合并。工程侧新增 chunk 级 `fact_level` / `source_ref` 导入，`KnowledgeReference` 携带 `factLevel` 与 `sourceClass`，运行时允许 `confirmed_facts`、`creative_materials`、`pending_clues` 全部进入写词 prompt；其中待核线索会带内部提示，要求作为暗线、传闻、情绪或意象使用，不写成官方定论。内容侧补充寻心、千夜、阿依努尔/阿依奴、田英、叶万山、小十七、道主、无相皇等高频人物卡，并新增 5 条 eval case。商用知识库当前为 68 个实体、97 个 chunk、42 条 eval case。验证通过：`STRICT_COMMERCIAL=1 python3 scripts/knowledge/validate-yanyun-knowledge.py knowledge-base/commercial-final`、导入脚本 `DRY_RUN=1`、Python `py_compile`、`:modules:knowledge:test`、`:modules:lyrics:test --tests '*DefaultLyricsGenerationServiceTest'`、`spotlessCheck`、`git diff --check`。本轮未调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker 或外部 embedding API。 |
| 2026-06-13 23:35 CST | 完成 P0-P4 燕云创作知识库百科重型补齐 | 按用户要求一次性补齐人物、剧情、地域、势力、玩法和回归题库，由 5 个只读子 Agent 并行产出草案，主 Agent 统一合并入 `knowledge-base/commercial-final/`。商用知识库保持 68 个实体，创作 chunk 从 97 扩到 231，eval case 从 42 扩到 131；高频人物已具备多卡人物传，核心剧情线拆出起点/冲突/转折/留白，地域和势力补足玩家事件、组织气质和禁写误区，玩法体验转成普通玩家故事素材。`pending_clues` 继续允许进入写词 prompt，但必须作为暗线、传闻、旧事、情绪或意象使用，不写成官方定论。同步更新知识库规格、来源登记和自问自答工作单；验证通过：严格知识库校验、导入 dry-run、脚本 `py_compile`、`:modules:knowledge:test`、`:modules:lyrics:test --tests '*DefaultLyricsGenerationServiceTest'`、`spotlessCheck`、`git diff --check`。本轮未调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker 或外部 embedding API。剩余为 P5 A/B 评测证据包、公司资料覆盖和品牌禁区校准。 |
| 2026-06-13 23:59 CST | 完成燕云知识库缺口盘点表 v0.1 | 按用户要求将“缺口盘点表”本身一次性补完整：新增 `docs/knowledge/yanyun-knowledge-gap-inventory-v0.1.md` 和 `knowledge-base/commercial-final-gap-inventory.json`，覆盖当前商用库基线 `68` 个实体、`231` 个 chunk、`131` 条 eval case，并整理出 `224` 行后续补库作战清单：人物 `76` 行、剧情 `30` 行、地域 `41` 行、势力 `29` 行、玩法 `27` 行、创作边界 `13` 行、eval `8` 行。表内每行均包含 `gap_id`、分类、状态、优先级、上线状态、建议来源等级、来源候选、建议补卡、写词影响、风险边界和目标分域文件；江南/杭州、打马棋/侠客偶棋等截至 `2026-06-13` 仍列为未来/预告资料，不进入当前 live 主库事实层。本轮只新增盘点文件，不修改正式知识库正文，不调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker 或外部 embedding API。验证通过：JSON 格式化、字段完整性/source id 引用检查、敏感信息扫描和 `git diff --check`。 |
| 2026-06-13 23:59 CST | 完成人物缺口盘点表 v0.2 补漏审计 | 针对 v0.1 人物组子 Agent 未完整返回导致的漏项风险，使用 5 个只读子 Agent 交叉调研官方/商店/公告、维基百科角色表、GameKee/灰机/攻略站、伙伴系统和 NPC 好感名单，由主 Agent 统一合并。人读版升级为 `docs/knowledge/yanyun-knowledge-gap-inventory-v0.2.md`，机器版 `knowledge-base/commercial-final-gap-inventory.json` 升为 `yanyun-knowledge-gap-inventory-v0.2`；人物行从 `76` 增至 `130`，整体 inventory 从 `224` 增至 `278`。新增赵光义/晋中原、张彦霖、周蔷、画睛兄、兰澳、薛丑、洪肆、魏仁浦、冯继昇、燕/应宁、蓬山四海豪客、四猫三鹅伙伴群像、墨山道代号人物和清河/开封/河西/青州普通 NPC 群像等缺口；陈子奚、玉山君、陈氏、钱王/杭州传说、文津馆十相仍按截至 `2026-06-13` 的官方预告口径放入 future/watchlist，不进入 live 人物事实层。本轮不修改正式知识库正文，不调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker 或外部 embedding API。验证通过：JSON 格式化、字段/source 引用校验、预告人物未误入 live 人物表检查、敏感信息 diff 扫描和 `git diff --check`。 |
| 2026-06-14 00:40 CST | 完成正式人物知识库第一批补库 | 已先提交缺口盘点表 v0.2 快照 `14f899f docs: refine yanyun character gap inventory`。随后按 v0.2 清单将第一批高影响人物和群像转入正式 `knowledge-base/commercial-final/characters.json`：新增赵光义/晋中原、张彦霖、周蔷、魏仁浦、薛丑、洪肆、冯继昇、燕/应宁、蓬山四海豪客、扫地门童、四猫三鹅伙伴群像，以及清河乡野营生、开封坊巷百业、河西/凉州边塞行旅、蓬山文津馆学子等普通 NPC 群像；新增 9 条 eval case 覆盖双名身份、河神祠悼亡、醉花阴/樊楼、九流市井、墨山道冷理性、蓬山豪客、四猫三鹅陪伴和普通玩家故事。正式商用知识库当前为 `87` 个实体、`272` 个 chunk、`140` 条 eval case；缺口盘点表已回写第一批覆盖状态，人物缺失从 `90` 降至 `71`。本轮未调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker 或外部 embedding API。验证通过：严格知识库校验、导入 dry-run、脚本 `py_compile`、Markdown/JSON 一致性、敏感信息 diff 扫描和 `git diff --check`。 |
| 2026-06-14 01:25 CST | 完成正式人物知识库第二批 P0 补库 | 按缺口盘点表继续推进第二批人物正式入库：新增姚药药、慧药、柴荣、王清、李祚、盈盈/温无缺、沈义伦、麻布袋、郭昕、张议潮、唐新词 11 个 P0 人物实体，每个实体补 5 张创作卡；对已有鬼市子、天不收追加边界和歌曲结构卡，分别从 `需拆分` / `偏薄` 回写为 `已有`。新增 13 条角色检索 eval case。正式商用知识库当前为 `98` 个实体、`331` 个 chunk、`152` 条 eval case；人物缺口盘点为 `130` 行，其中 `已有 48 / 缺失 60 / 仅待核线索 22`。本轮未调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker 或外部 embedding API。验证通过：严格知识库校验、导入 dry-run、脚本 `py_compile`、JSON 统计检查、敏感信息 diff 扫描、`git diff --check`；待继续运行 Java 目标测试并提交快照。 |
| 2026-06-14 01:55 CST | 完成正式人物知识库第三批 P1 补库 | 继续按缺口盘点表推进第三批人物正式入库：新增画睛兄、兰澳、贺然、黎蓁蓁、柳青衣、柏楚玉、福禄寿三姐妹、赵大哥、龟奶奶、史鸩、石守信、慕容延钊 12 个 P1 人物实体，每个实体补 4 张创作卡；覆盖文津馆、无心谷、清河/开封市井、势力代表和开封庙堂线。新增 12 条角色检索 eval case。正式商用知识库当前为 `110` 个实体、`379` 个 chunk、`164` 条 eval case；人物缺口盘点为 `130` 行，其中 `已有 60 / 缺失 48 / 仅待核线索 22`。本轮未调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker 或外部 embedding API。验证通过：严格知识库校验和 `git diff --check`；待继续跑导入 dry-run、脚本编译、Java 目标测试并提交快照。 |
| 2026-06-14 02:25 CST | 完成正式人物知识库第四批 P1 补库 | 继续按缺口盘点表推进第四批人物正式入库：新增朱鱼、冯夷、赵承宗、廉道子、张淮深、望月婵媛、安琉璃、鹰铃、宝音、方白、宋九薇、燕十七 12 个 P1 人物实体，每个实体补 4 张创作卡；覆盖开封暗线、河西/凉州、归义军、青州/文津馆和伙伴人物线。新增 12 条角色检索 eval case。正式商用知识库当前为 `122` 个实体、`427` 个 chunk、`176` 条 eval case；人物缺口盘点为 `130` 行，其中 `已有 72 / 缺失 36 / 仅待核线索 22`。本轮未调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker 或外部 embedding API。验证通过：严格知识库校验、导入 dry-run、脚本 `py_compile`、JSON/Markdown 统计检查、`:modules:knowledge:test`、`:modules:lyrics:test --tests '*DefaultLyricsGenerationServiceTest'` 和 `git diff --check`。 |
| 2026-06-14 02:50 CST | 完成正式人物知识库第五批 P1 收尾 | 继续按缺口盘点表推进第五批人物正式入库：新增小猫佑安、白狼主、花魁、叶墨城、福宝、时一墨 6 个 P1 人物实体，每个实体补 4 张创作卡；覆盖伙伴陪伴、审美人物、青州伙伴线和墨山道机关线。新增 6 条角色检索 eval case。正式商用知识库当前为 `128` 个实体、`451` 个 chunk、`182` 条 eval case；人物缺口盘点为 `130` 行，其中 `已有 78 / 缺失 30 / 仅待核线索 22`，P1 人物缺口已清空。本轮未调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker 或外部 embedding API。验证通过：严格知识库校验、导入 dry-run、脚本 `py_compile`、JSON/Markdown 统计检查、`:modules:knowledge:test`、`:modules:lyrics:test --tests '*DefaultLyricsGenerationServiceTest'` 和 `git diff --check`。 |
| 2026-06-14 03:20 CST | 完成正式剧情知识库第一批 P0 补库 | 从人物层转向剧情线补库，新增清河主章·神仙不渡、不羡仙旧事、开封主章·天地熔炉、无忧洞、河西主章·问月长安 5 条 P0 剧情实体，每条实体补 5 张创作卡；覆盖归家/火后、不羡仙旧事、开封大城如炉、地下开封救援、河西问月长安与归义军旧事。为避免和地点实体别名冲突，剧情实体使用“不羡仙旧事”而不是直接复用“不羡仙”。新增 5 条剧情检索 eval case。正式商用知识库当前为 `133` 个实体、`476` 个 chunk、`187` 条 eval case；剧情缺口盘点为 `30` 行，其中 `已有 11 / 缺失 8 / 偏薄 1 / 需拆分 10`。本轮未调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker 或外部 embedding API。验证通过：严格知识库校验、导入 dry-run、脚本 `py_compile`、JSON/Markdown 统计检查、`:modules:knowledge:test`、`:modules:lyrics:test --tests '*DefaultLyricsGenerationServiceTest'` 和 `git diff --check`。 |
| 2026-06-14 04:05 CST | 完成知识库剩余缺口一次性补齐 | 按用户要求将缺口盘点表中剩余 `155` 条非 `present` 项一次性转入正式商用知识库或 eval 题库：新增人物 `52`、剧情 `19`、地域 `31`、势力 `19`、玩法 `20`、创作边界 `6` 个实体，新增 `441` 张创作卡和 `149` 条 eval/召回题；正式库当前为 `280` 个实体、`917` 个 chunk、`336` 条 eval case，`commercial-final-gap-inventory.json` 的 `278` 行缺口均已回写为 `present`。`pending_clues` 仍允许进入写词 prompt，但只能作为暗线、传闻、旧事、情绪或意象使用，不写成官方定论。同步更新缺口盘点人读版 v0.3 和来源登记。验证通过：严格知识库校验、导入 dry-run、脚本 `py_compile`、`:modules:knowledge:test`、`:modules:lyrics:test --tests '*DefaultLyricsGenerationServiceTest'`、`spotlessCheck`、`git diff --check` 和敏感信息扫描。本轮未调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker 或外部 embedding API。 |
| 2026-06-14 04:25 CST | 生成知识库 P5 离线证据包 | 在缺口表全部补齐后，新增 `scripts/knowledge/generate-yanyun-kb-evidence.py`，可离线生成知识库交付证据，不调用真实模型、不联网、不导入数据库。已生成 `docs/knowledge/yanyun-knowledge-p5-evidence-v0.1.md`、`knowledge-base/commercial-final-provenance-manifest.json` 和 `knowledge-base/commercial-final-pollution-audit.json`：证据包确认当前正式库为 `280` 个实体、`917` 个 chunk、`336` 条 eval case，缺口表剩余非 `present` 为 `0`；污染审计结果为实体问题 `0`、chunk 问题 `0`、eval 引用问题 `0`、敏感信息问题 `0`。同步更新知识库规格、来源登记和自问自答工作单。验证通过：证据脚本、严格知识库校验、导入 dry-run、JSON 格式化、脚本 `py_compile`、目标 Java 测试、`spotlessCheck`、`git diff --check` 和敏感信息扫描。本轮未调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker 或外部 embedding API。 |
| 2026-06-14 12:01 CST | 导入商用知识库并完成离线 A/B 检索评测 | 检查确认当前商用知识库已经足够进入 A/B：缺口清单 `278` 行均为 `present`，正式库为 `280` 个实体、`917` 个 chunk、`336` 条 eval case，污染审计和敏感信息审计均为 `0`。导入脚本改为同版本幂等重写，已将 `yanyun-commercial-kb-2026-06-13-v1` 导入本地 PostgreSQL/pgvector，库内确认 `280` 个实体、`917` 个 chunk、`906` 个别名。修复检索策略：实体 alias 命中按实体去重，并按命中实体均分上下文名额；补充真实用户表达别名，修正补充拆分卡 eval 口径。新增离线 A/B 脚本和报告，48 条样本中 pgvector 组 any-hit 为 `100%`、all-hit 为 `68.75%`、P50 `248.46ms`、P95 `329.5ms`，速度达标；剩余 partial 主要是角色题希望顺带召回地域/剧情等关系扩展，属于下一阶段关系图检索优化，不阻塞进入真实模型 A/B。当前 shell 没有 `DEEPSEEK_API_KEY`，所以本轮没有调用真实 DeepSeek，只完成检索/上下文层 A/B。验证通过：严格知识库校验、证据生成、真实导入、离线 A/B、脚本 `py_compile`、`:modules:knowledge:test`、`:modules:lyrics:test --tests '*DefaultLyricsGenerationServiceTest'`、`spotlessCheck` 和 `git diff --check`。 |
| 2026-06-14 12:30 CST | 完成 DeepSeek 真实歌词 A/B | 用户提供 DeepSeek 凭据后，新增并执行 `scripts/knowledge/run-yanyun-kb-real-lyrics-ab.py`，以 4 条代表样本对比 A 组“禁用知识库”和 B 组“pgvector 知识库上下文”。本轮只调用 DeepSeek 文本模型，不调用 Yunwu、WellAPI、DreamMaker，不生成歌曲/封面/视频；报告和 JSON 结果不保存完整歌词、完整 prompt、供应商原始响应、密钥或媒体 URL，只保存标题、摘要、hash、结构统计和评审分数。结果：B 组胜 `3`，A 组胜 `1`，平局 `0`；平均 overall 从 A `6.83` 提升到 B `8.45`，平均燕云世界归属感从 A `6.25` 提升到 B `8.0`。A 组胜出的样本是“九流门局中局 + 普通小摊贩”，说明知识库 B 组在该题过度偏势力资料，市井细节反而不足；下一步应补轻量关系扩展/多样性召回，让 `九流门` 同时带出 `开封市井/夜市/普通玩家故事`。验证通过：真实 A/B preflight、真实 A/B execute、结果 JSON 格式化、脚本 `py_compile` 和 `git diff --check`。 |
| 2026-06-14 12:45 CST | 重跑真实歌词 A/B 并改为人工评审 | 按用户要求重跑同样 4 条真实 DeepSeek 歌词 A/B，但本轮 DeepSeek 只负责生成，不再调用 DeepSeek 评审。`scripts/knowledge/run-yanyun-kb-real-lyrics-ab.py` 新增 `MODE=execute_manual_review`，完整歌词和完整写词 Prompt 写入本地忽略目录 `build/reports/knowledge/yanyun-real-lyrics-ab-manual-review.md` 和 JSON 结果，避免进入 Git。人工初评：B 组知识库上下文在 4 条样本中均优于 A 组，尤其寒香寻、阿依努尔和普通弟子三题明显减少泛古风并增强角色/地域/小人物故事感；九流门小摊贩题 B 组更稳住普通人视角，但仍缺少 `九流火气` 剧情线召回，需要后续做关系扩展检索。验证通过：真实 A/B preflight、真实 A/B manual review execute、手动评审 JSON 格式检查、脚本 `py_compile` 和 `git diff --check`。本轮只调用 DeepSeek 文本模型，不调用 Yunwu、WellAPI、DreamMaker，不生成歌曲/封面/视频。 |
| 2026-06-14 13:30 CST | 收束知识库写词链路并跑通知识库真实完整样本 | 已先按功能块提交当前成果：商用知识库内容/缺口/证据包、pgvector 导入/校验/检索修复、离线与真实歌词 A/B 报告；随后微调 `RealDeepSeekLyricsClient` 与真实歌词 A/B 脚本 Prompt，强化“可重复 hook、具体动作/物件/场景、小人物口吻、少堆万能古风词”。九流门关系检索通过补桥接别名解决，`eval-heavy-story-003-jiuliu-common` 现在同时召回 `story-jiuliu-huoqi`、`faction-jiuliumen` 和普通人素材；离线 A/B any-hit `100%`、all-hit `70.83%`、P50 `249.48ms`、P95 `447.53ms`。本地真实完整 smoke 默认启用 `KNOWLEDGE_RETRIEVAL_MODE=pgvector` 和商用 KB 版本，ADR/规格/README 已同步。真实完整样本 `bba0c628-641d-4aac-80de-110cdb8a75e7` 使用 DeepSeek v4Pro + pgvector KB + Yunwu Suno `chirp-fenix` + WellAPI Image2 + `album-ffmpeg` 达到 `GENERATED / PACKAGE_READY`，KnowledgeRetrieve、CreativeBrief、Lyrics、MusicPrompt、CoverPrompt、QualityEvaluation 均成功，音频/封面/视频/timeline 均入库，发布包 URL 可读，MP4 H.264/AAC 双轨通过。首次 Claude Web v1 成品页 smoke 暴露前端 `mock_user_001` 被错误判非法并随机换用户，已修复前端校验和 smoke localStorage 初始化；复测同一作品成品页、媒体入口、已交接状态、390px/1440px 无横向溢出通过。验证通过：严格知识库校验、导入 dry-run/真实导入、证据生成、离线 A/B、相关 Python 编译、`:modules:knowledge:test`、`:modules:deepseek:test --tests '*RealDeepSeekLyricsClientTest'`、`:modules:lyrics:test --tests '*DefaultLyricsGenerationServiceTest'`、`spotlessCheck`、Claude Web v1 `test/typecheck/build`、脚本语法和 `git diff --check`。本轮真实样本只跑 1 条，不记录密钥、完整 Prompt、完整歌词、供应商原始响应或媒体 URL；DreamMaker 仍是生产目标，Yunwu/WellAPI 仍仅作为公网联调路径。 |
| 2026-06-14 14:40 CST | 落地作词 Agent v0.6 工艺收束 | 按用户确认，知识库检索和注入方式保持不变，只优化“歌词更像歌”。`RealDeepSeekLyricsClient` 新增内部作词规划、主韵脚、整首不得完全无韵、中文 7-14 字短句、副歌 hook 绑定韵脚、避免散文化叙事流水账和硬凑押韵等约束；`RealDeepSeekQualityEvaluationAgent` 的 LYRICS 质量门新增副歌韵脚、近韵自然度、句长可唱性和 hook 声音记忆点评估，故事清楚但不像歌时不应给高分；`CreativeBriefAgent` 只在既有 `narrative_viewpoint` 中补充语言口吻建议，不新增字段、不改变创作域判断。真实歌词 A/B 脚本同步 v0.6 写词 prompt 与人工评审维度，完整 Prompt/歌词仍只允许写入 ignored 的 `build/reports/knowledge/`。验证通过：`:modules:deepseek:test --tests '*RealDeepSeekLyricsClientTest' --tests '*RealDeepSeekCreativeAgentsTest'`、`:modules:lyrics:test --tests '*DefaultLyricsGenerationServiceTest'`、`python3 -m py_compile scripts/knowledge/run-yanyun-kb-real-lyrics-ab.py`、`spotlessCheck`、`git diff --check`。本轮未调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker，未新增依赖，未改 OpenAPI/数据库/前端/知识库检索逻辑。 |
| 2026-06-14 14:51 CST | 提交作词 Agent v0.6 并尝试真实 A/B preflight | 已提交快照 `41122854 feat: tune lyrics agent songwriting craft v0.6`。随后尝试运行真实歌词 A/B preflight，脚本按安全门阻断执行，当前 shell 缺少 `ALLOW_REAL_MODEL_SMOKE=1`、`ALLOW_DEEPSEEK_REAL_AB=1` 和 `DEEPSEEK_API_KEY`；未调用真实 DeepSeek，未生成新的 A/B 样本。下一步需要在当前 shell 以安全方式注入 DeepSeek key 并打开双 gate 后，再执行 4 条 v0.6 真实歌词 A/B。 |
| 2026-06-14 15:04 CST | 完成 v0.6 真实歌词 A/B 人工评审 | 用户再次授权并提供 DeepSeek 凭据后，使用隐藏输入方式临时注入当前 shell，只打开 `ALLOW_REAL_MODEL_SMOKE=1` + `ALLOW_DEEPSEEK_REAL_AB=1`，执行 4 条 v0.6 真实歌词 A/B `execute_manual_review`。本轮只调用 DeepSeek v4Pro 生成歌词，不调用 Yunwu、WellAPI、DreamMaker，不生成歌曲/封面/视频；完整 Prompt 和完整歌词仅写入 ignored 的 `build/reports/knowledge/yanyun-real-lyrics-ab-manual-review.*`。人工评审结论：B 组 pgvector 知识库上下文 `4/4` 优于 A 组，v0.6 在副歌 hook、韵脚意识和短句可唱性上明显改善，普通弟子样本《残招作舟》是本轮最佳；仍需后续注意角色歌的高级转折、减少“月光/流浪/故乡/江湖/长夜”等万能词，并约束模型创造的私人招式/物件不得被写成官方技能或官方定论。 |
| 2026-06-14 15:28 CST | 补充作词 Agent 万能词密度约束 | 按用户澄清，本轮不限制私人化歌词意象，也不改“自创生活物件/私人称呼”的策略；只加强万能诗性词密度控制。`RealDeepSeekLyricsClient` 和真实 A/B 脚本新增约束：允许使用“明月、山河、江湖、风烟、长夜、流浪、故乡”等词，但不能连续当填充词堆叠，出现时应落到具体动作、物件或场景上；同段超过 3 个万能词时要求内部重写为更具体的燕云画面、人物动作或生活细节。`QualityEvaluationAgent` 的 LYRICS gate 同步检查万能词是否连续堆叠成填充词，模板版本升为 `quality.evaluation.v6.1`。验证通过：`:modules:deepseek:test --tests '*RealDeepSeekLyricsClientTest' --tests '*RealDeepSeekCreativeAgentsTest'`、`python3 -m py_compile scripts/knowledge/run-yanyun-kb-real-lyrics-ab.py`、`spotlessCheck`、`git diff --check`。本轮未调用真实模型。 |
| 2026-06-14 15:41 CST | 跑通 v0.6.1 知识库完整真实产品样本 | 按用户继续推进，先提交万能词密度补丁 `34bd492f feat: tighten generic lyric imagery density`，随后通过统一入口执行 1 条 `public-real-full-experience` 真实完整样本。脚本以隐藏输入方式临时注入 DeepSeek/Yunwu/WellAPI 三组凭据并打开双 gate；preflight 为 `READY`。作品 `f334adfc-155c-4595-95d3-6f2dfc1d1fd9` 从真实 DeepSeek v4Pro + pgvector 写词进入 Yunwu Suno `chirp-fenix` 出歌、WellAPI Image2 封面、`album-ffmpeg` MP4 成片，最终达到 `GENERATED / PACKAGE_READY`，随后前端 smoke 刷新素材链接并标记交接，达到 `PACKAGE_FETCHED`。脱敏证据显示 KnowledgeRetrieve、CreativeBrief、Lyrics、LyricsQualityGate、MusicPrompt、MusicQualityGate、CoverPrompt、CoverQualityGate、PackageQualityGate 均成功；音频、封面、视频、timeline 均入库，封面为 `2048x1152` JPEG，MP4 为 `1920x1080` H.264/AAC，Claude Web v1 成品页、音频/封面/视频入口、刷新链接和交接状态均通过。脚本启动的 API/worker/frontend 已自动停止，`8080/8081/5274` 无监听残留。非阻塞风险：本地 PostgreSQL 输出 collation version mismatch warning，后续可在维护窗口处理数据库 collation 刷新；本轮不影响生成链路。DreamMaker 仍是生产目标，Yunwu/WellAPI 仍仅作为公网联调路径。 |
| 2026-06-14 16:21 CST | 收束创作首页输入项与音乐偏好 | 按用户确认，将创作入口从“填表式创作”改为“自由灵感 + 心情 + 少量音乐控制”：Claude Web v1 灵感成歌删除显性 `scene` / `relationship` 字段，保留文本式 `mood`；曲风 preset 改为开放音乐类型（流行抒情、R&B、摇滚、民谣叙事、说唱、电子、影视OST、史诗战歌、City Pop、国风融合），不再默认国风；声线偏好改为智能匹配、男声主唱、女声主唱、双人对唱、副歌和声点缀。OpenAPI 与后端创建请求同步移除公开 `scene` / `relationship`，`vocal_preference` 改为开放字符串；数据库历史列保留并写空，旧客户端未知字段显式 ignore。Suno/DreamMaker 人声映射兼容 `MALE/FEMALE` 与新 `MALE_LEAD/FEMALE_LEAD`，其他声线作为 MusicPromptAgent 的开放偏好处理。同步更新 smoke 样例、前端任务包和技术方案；`openapi-contract.sh` 顺手修正确认出歌必须绑定当前 `lyrics_draft_id`，并将未开放的视频重渲染断言改为受控 501。验证通过：Claude Web v1 `npm test` / `typecheck` / `build`，390px/1440px Playwright 首页无溢出且旧字段消失，独立 18080 mock API 上 `scripts/smoke/openapi-contract.sh` 通过，`:apps:music-api:test`、`:modules:deepseek:test --tests '*RealDeepSeekCreativeAgentsTest'`、`:modules:suno:test`、`spotlessCheck`、脚本 `bash -n` 和 `git diff --check` 通过。本轮未调用真实 DeepSeek、Yunwu、WellAPI 或 DreamMaker。 |
| 2026-06-14 17:19 CST | 恢复本地真实测试环境 | 用户在 Claude Web v1 创作时遇到 `Bad Gateway`；复现确认 5275 前端仍在，但 8080 API / 8081 worker 未监听，Vite 代理无法连接后端。Docker 基础设施正常，三组真实模型凭据在 `.env.real.local` 中已配置且 strict preflight 为 `READY`。已用 detached `screen` 会话启动 `yanyun-api` 与 `yanyun-worker`，配置为 DeepSeek v4Pro + pgvector KB + Yunwu Suno `chirp-fenix` + WellAPI Image2 + MinIO/S3 + `album-ffmpeg`，公司 Adapter 仍为 mock；验证 `http://127.0.0.1:8080/health` 与 `http://127.0.0.1:8081/actuator/health` 均正常，`http://127.0.0.1:5275/api/v1/me` 经前端代理返回 200。当前用户可继续在 `http://127.0.0.1:5275/` 做真实模型测试；生成作品会产生真实供应商调用成本。 |
| 2026-06-14 18:12 CST | 优化成品页作品预览布局 | 按用户反馈将 Claude Web v1 成品页从“左封面 + 右音频/视频”改为“作品预览优先”：主卡左侧为 16:9 视频播放器并使用封面作 poster，右侧展示完整歌词，音频改为低权重“仅听音频”折叠入口，封面只保留为 poster 和交接素材入口。歌词展示优先使用 `work.lyrics_draft.lyrics_text`，缺失时 fallback 到 `publishPackage.package_json.lyrics.text`，并增加“复制歌词”按钮；继续不展示 package JSON、内部完整 URL 或 music prompt。成品页单独放宽内容宽度，840px 以上视频/歌词并排，移动端纵向排列。验证通过：Claude Web v1 `npm test` 11 个测试文件 / 39 个测试通过、`npm run typecheck`、`npm run build`、Playwright 检查 390px / 919px / 1440px 当前作品页无 console error、无横向溢出，视频与歌词顶线对齐。 |
| 2026-06-14 22:43 CST | 修复成品页歌词复制兜底 | 用户在 Claude Web v1 成品页点击“复制歌词”无效；定位为嵌入式浏览器可能暴露 `navigator.clipboard` 但因权限或焦点拒绝写入，旧逻辑不会再尝试 textarea 兜底。已将复制逻辑改为 Clipboard API 失败后自动使用隐藏 textarea + `execCommand('copy')` 兜底，失败时提示用户长按手动复制，并新增回归测试覆盖权限失败场景。验证通过：`npm test -- FinishedView.test.tsx`、`npm run typecheck`、`npm run build`。 |
| 2026-06-14 23:02 CST | 修复润色/续写弹窗逐字断焦 | 用户反馈 `AI 润色歌词` 弹窗 textarea 每打一个字就断焦；用当前作品 `19ae2aa0-7b7b-46c3-a879-ef493e9b0477` 的本地 mock user 复现后确认真实根因是 `Modal` 的焦点管理 effect 依赖了每次 render 都变化的 `onClose`，输入一个字触发 re-render 后 cleanup 会把焦点还给弹窗外按钮，导致输入法/光标中断。已改为用 ref 保存最新 `onClose`，Modal 打开期间 effect 只随 `open` 生命周期运行；同时保持子控件已聚焦时不抢焦点、润色/续写 textarea 自动聚焦、空输入只禁用按钮不提前报错，纯空格才提示。Playwright 复测同一作品：逐字输入完整保留，active element 仍是 `TEXTAREA`，计数 `13/500`，“开始润色”解禁。验证通过：`npm test -- ConfirmView.test.tsx`、`npm run typecheck`、`npm run build`、`git diff --check`。 |
| 2026-06-14 23:18 CST | 修复 AI 润色 DeepSeek 空响应导致 500 | 用户在作品 `19ae2aa0-7b7b-46c3-a879-ef493e9b0477` 使用 `AI 润色歌词` 时出现 `Internal server error`，请求编号 `8a65a3e3-091d-4a96-9dc6-1b2c4ad37e07`。日志定位为 `CreativeBriefAgent` 调 DeepSeek 连续返回空 `message.content` 后抛出 `IllegalStateException: DeepSeek response content is empty`，API 未收敛成可用体验。已修复：`DefaultLyricsGenerationService` 在 CreativeBriefAgent 失败时降级生成本地最小 creative brief，继续使用用户润色指令、当前歌词和知识库上下文完成写词；`RealDeepSeekLyricsClient` 增加空 `choices/message.content` 的语义重试，避免最终 LyricsAgent 因一次空包立刻失败。已重启本地 `music-api` 让修复生效，`http://127.0.0.1:8080/health` 返回 OK；只读确认该作品 `polish_used_count=0`，本次失败未消耗改词次数。验证通过：`:modules:lyrics:test --tests '*DefaultLyricsGenerationServiceTest'`、`:modules:deepseek:test --tests '*RealDeepSeekLyricsClientTest'`、`spotlessCheck`、`:apps:music-api:bootJar`。本轮未额外发起真实润色请求，避免消耗用户改词次数和 DeepSeek 调用。 |
| 2026-06-15 00:35 CST | 落地作词 Agent v0.7 开放创作判断力 | 按用户确认，不改知识库检索/注入方式、不改前端/OpenAPI/数据库，只升级创作 Agent 的作词判断力。`CreativeBriefAgent` 从固定 brief 扩展为开放创作可能性判断，新增 `creative_core`、`chosen_angle`、`alternative_angles`、`anti_cliche_strategy`、`voice_texture`、`image_pool`、`song_energy` 等内部字段；`LyricsAgent` 支持叙事、意象、口语、对白/独白、群像、反差、重复、反讽、留白、反套路等多路径自主选择，hook 不再限定为金句，曲风只作为声口/节奏参考；`QualityEvaluationAgent` 改为按“是否值得被唱”审稿，完整但普通、泛江湖第一反应、有剧情但不像歌、没有声音记忆点时要求 `REWRITE`；一次重写按 `rewrite_angle` / `rewrite_cliche` / `rewrite_voice` / `rewrite_memory_point` / `rewrite_singability` / `rewrite_grounding` 定向追加指令，自评分低质阈值提升到 `0.80`。验证通过：`:modules:deepseek:test --tests '*RealDeepSeekLyricsClientTest' --tests '*RealDeepSeekCreativeAgentsTest'`、`:modules:lyrics:test --tests '*DefaultLyricsGenerationServiceTest'`、`:modules:deepseek:test :modules:lyrics:test`、`spotlessCheck`、`git diff --check`。本轮未调用真实 DeepSeek、Yunwu、WellAPI、DreamMaker，未生成音乐/封面/视频。 |
| 2026-06-15 01:06 CST | P0 收口：提交、重启本地真实环境并完成 v0.7 文本 A/B | 按用户 P0 一次性收口计划，已先完成安全与基线检查：`git diff --check`、敏感信息扫描、OpenAPI mock smoke、Java 目标测试、`spotlessCheck`、API/worker `bootJar`、Claude Web v1 `npm test/typecheck/build`、严格知识库校验与 dry-run 均通过。已按功能块提交 3 个代码快照：`8b73e7ec feat: simplify creation controls and finished preview UX`、`54b785ba feat: resolve yanyun entity typo candidates`、`316ffc44 feat: upgrade lyrics grounding and songwriting judgment v0.7`。随后修正真实歌词 A/B 脚本，使其与 v0.7 生产 Prompt 对齐：不再默认国风，不再把固定金句 hook 当唯一标准，继续只把完整 Prompt/歌词写入 ignored 的 `build/reports/knowledge/`。已重新导入 `yanyun-commercial-kb-2026-06-13-v1` 到本地 pgvector，并重启本地真实 API/worker，保持 5275 前端不变；验证 `8080/health`、`8081/actuator/health` 和 `5275/api/v1/me` 均正常。DeepSeek 文本 A/B 已用双 gate 执行 8 条样本，未调用 Yunwu、WellAPI、DreamMaker，未生成音乐/封面/视频。结果摘要：实体纠错链路有效，`姜无浪` 样本召回 `江无浪`，`若水岸` 样本召回 `弱水岸/寻心`；B 组 pgvector 在寒香寻、弱水岸寻心、小门派弟子等题上明显增强世界归属和事实接地，九流门/奇术情歌能保持普通人或轻快视角；仍需后续人工继续评估“更稳但不一定更有灵气”的样本，避免知识库让歌词过长或过保守。本轮剩余待提交范围为 A/B 脚本同步和项目进度文档，完整 Prompt/完整歌词报告仍未进入 Git。 |
| 2026-06-15 01:19 CST | 启动朋友公网临时试用入口 | 按用户要求重新开启可公网访问的临时试用环境：保留本地 Claude Web v1 前端和 Docker 基础设施，使用 Cloudflare 临时 tunnel 暴露前端与 MinIO 媒体文件；API/worker 已重启并配置为 DeepSeek v4Pro + pgvector KB + Yunwu Suno `chirp-fenix` + WellAPI Image2 + `album-ffmpeg` + MinIO/S3，公司账号/审核/权益/发布/分享仍为 Mock Adapter。已验证公网前端返回 200，公网前端代理 `/api/v1/me` 返回 200；刷新既有 mock 作品发布素材链接后，发布包 URL host 已切到公网媒体 tunnel，Range GET 返回 206，说明朋友公网环境可加载新签名素材。临时公网 URL 只在对话中提供，不写入仓库文档；风险：任何拿到链接的人都可发起真实生成并消耗当前供应商额度，且该环境依赖本机、Docker、screen 和 Cloudflare tunnel 持续运行。 |
| 2026-06-15 01:29 CST | 修复查看作品时短暂闪现“状态无法识别” | 用户反馈从“我的作品”打开作品详情时会先闪现 `作品状态无法识别`，随后又正常进入歌词页。定位为 Claude Web v1 `useWorkDetail` 的请求竞态：作品详情页切换或 React 开发模式 effect 重放时，旧请求被 abort 后仍在 `finally` 中执行 `setLoading(false)`，导致页面短暂进入 `work=null / loading=false / error=null` 的空洞状态，被 WorkPage 兜底渲染为未知状态。已为作品详情请求增加递增序号和 current request guard，只有最新请求才能写入 `work/error/loading`；被 abort 或过期的请求不再影响页面。新增 `WorkPage.test.tsx` 回归测试，覆盖旧请求 abort 后页面必须保持 `正在打开作品`，不得闪现未知状态。验证通过：Claude Web v1 `npm test` 12 个测试文件 / 43 个测试、`npm run typecheck`、`npm run build`、`git diff --check`。 |
| 2026-06-15 03:24 CST | 完成 P1#2-#6 + P2/P3 真实测试可信修复 | 按用户指定范围，本轮不处理 P0 公司 Adapter，也不修 P1#1 公网访问控制/限额，集中收束真实测试可信性。后端一致性：新增 `QUOTA_COMMIT_FAILED`，配额 commit 失败不再进入 `GENERATED / PACKAGE_READY`；润色/续写写草稿改为基于 `works.version` 的乐观锁更新；`publish-package/refresh-url` 保留 `PACKAGE_READY / PACKAGE_FETCHED` 状态并允许过期 fetched 包刷新；没有真实 retry endpoint 的失败码不再下发假 retry 语义。安全边界：CreativeBrief fallback 改为本地创作域 fail-closed，CreativeBrief 与质量门共用其他 IP / 风险词表；API 未知异常日志改为脱敏 message；封面 metadata 只保留 provider option 白名单；`/internal/integration-readiness` 默认只允许本地请求。媒体与前端：真实完整 smoke 强制 `OBJECT_STORAGE_PROVIDER=s3`、`S3_PUBLIC_ENDPOINT`、`RENDER_WORKER_MODE=album-ffmpeg`，避免 `local` 存储生成公网不可播放链接；demo 模式不再伪造 MP4；成品页交接区素材链接改用 publish package JSON；`PACKAGE_BLOCKED` 展示阻断原因；歌词确认页增加复制歌词；移除前端无接口支撑的 `RETRY_LYRICS` 假动作；统一 5273/5274/5275 端口文档口径。知识库与脚本：source registry 增加锚点校验，证据脚本补死链/坏标点/未来资料扫描，真实 A/B 脚本复用生产 `RealDeepSeekLyricsClient` system prompt；`openapi-contract.sh` 增加运行时安全门，检测到 DeepSeek/Yunwu/WellAPI/非 mock render 时默认拒绝动态契约测试，避免误调用真实模型。真实样本验证：首条样本 `2b0e96d3-1131-40d1-b118-2b6be85da08f` 暴露封面 fallback 写 SVG 导致 FFmpeg 不支持解码，已改为 PNG fallback 并补测试；复跑样本 `6fefbd41-e886-4db7-9d0d-788e08def5f7` 达到 `GENERATED / PACKAGE_READY`，音频、封面、视频均存在，发布包素材 Range GET 均为 `206`，公网前端作品路由和 `/api/v1/me` 均返回 `200`。注意：S3 预签名 URL 的 `HEAD` 返回 `403` 属于方法签名限制，浏览器播放所需 GET/Range 路径已验证可用。验证通过：Claude Web v1 `npm test/typecheck/build`，严格知识库校验与导入 dry-run，Python 脚本编译，Java `:apps:music-api:test :apps:music-worker:test :modules:production:test :modules:knowledge:test :modules:lyrics:test :modules:deepseek:test :modules:suno:test :modules:image2:test`、`spotlessCheck`、API/worker `bootJar`，OpenAPI 静态契约和动态契约真实环境负向安全门，`git diff --check` 与敏感信息 diff 扫描。已重启本地真实 API/worker，readiness 为 DeepSeek real + Yunwu Suno + WellAPI Image2 + S3ObjectStorageClient + album-ffmpeg；公网临时前端/媒体 tunnel 仍在，适合可信小范围朋友测试，但由于 P1#1 限额/访问控制未修，任何拿到链接的人仍可能消耗真实供应商额度。 |
| 2026-06-15 04:12 CST | 修复封面 Prompt 质量门误拦截与收尾重试 | 用户实测作品 `3771ac68-e489-476e-9426-79c3eb8d82f9`（《宫灯看剑》）失败根因确认不是 Yunwu 音乐失败，而是音频已成功入库后，`COVER_QUALITY_GATE` 将“仅允许作品歌名主标题、无违规文本元素”的合规封面 Prompt 误判为非 PASS，导致工作流停在封面生成前。已修复 `RealDeepSeekQualityEvaluationAgent` 的 COVER 本地兜底：允许受控歌名主标题通过，继续阻断正向假歌手、假厂牌、假版权、水印、二维码、乱码、UI 等风险，并修复 reasons 中否定语境（如 disallow/未要求）被误判为正向风险的问题。新增正式收尾重试能力：`cover/regenerate` 对 `PACKAGE_BUILD_FAILED` 且已有 AUDIO 的作品会 reserve 后以 `reuseExistingAudio=true` 重新进入 workflow，跳过 MusicPrompt、MusicQuality 和音乐供应商调用，只补封面、视频、timeline 和发布包；worker 层也明确避免把复用音频误写成 mock 静音音频。已重启本地真实 API/worker 并对旧作品执行受控收尾重试，最终达到 `GENERATED / PACKAGE_READY`，audio、cover、video、timeline 均入库，publish package 中 audio/video Range GET 为 `206`、cover GET 为 `200`；`provider_calls` 只有原始一次 `MUSIC_GENERATION / SUNO / SUCCEEDED`，确认没有重新调用 Suno。验证通过：`:modules:deepseek:test --tests '*RealDeepSeekCreativeAgentsTest'`、`:apps:music-api:test --tests '*WorkServiceWorkflowDispatchTest' --tests '*MockSongProductionWorkflowTest'`、`spotlessCheck`、API/worker `bootJar`、健康检查、公网前端/API 200 和媒体 URL 检查。 |
| 2026-06-15 17:30 CST | 修复公网润色与确认出歌草稿同步问题 | 朋友公网实测暴露两类问题：一是 `AI 润色` 成功生成新歌词草稿后，前端仍可能带旧 `lyrics_draft_id` 调用确认出歌，后端正确拒绝并返回 `Lyrics draft is not the current confirmable draft`；二是公网润色链路中 DeepSeek 偶发空响应/坏 JSON，前端网络错误文案仍提示 `localhost:8080`，对公网用户误导。已修复 Claude Web v1 动作执行器，支持等待异步刷新；润色/续写成功后必须拉取最新作品详情再关闭弹窗和恢复“确认出歌”，确认出歌遇到旧草稿 409 时自动刷新并提示“歌词已更新，请重新确认出歌”；网络错误文案改为公网/本地都可理解的“作曲服务连接中断，请稍后重试”，不再暴露 `localhost:8080`。后端新增 `LyricsGenerationTransientException`，`RealDeepSeekLyricsClient` 将空 content、坏 content JSON、缺 `lyrics_text` 等语义内容失败纳入重试，重试耗尽后由 API 返回 `LYRICS_PROVIDER_TEMPORARY_FAILURE / 503`，提示“AI 润色/续写/写词暂时失败”，事务回滚，不生成草稿、不消耗改词次数。验证通过：Claude Web v1 `npm test` 12 个文件 / 48 个测试、`npm run typecheck`、`npm run build`，Java `:modules:deepseek:test --tests '*RealDeepSeekLyricsClientTest'`、`:modules:lyrics:test --tests '*DefaultLyricsGenerationServiceTest'`、`:apps:music-api:test`、`spotlessCheck`、API/worker `bootJar`、`git diff --check`。本轮未调用 Yunwu、WellAPI 或重新生成音乐/封面/视频。 |
| 2026-06-15 19:05 CST | 修复公网 AI 润色长请求断开后的假失败体验 | 用户实测作品 `351839a8-f0fe-4425-9b09-13e9a3ceec29` 使用 `AI 润色歌词` 等待较久后提示“作曲服务连接中断”。排查 API 日志确认 `/lyrics/polish` 在写回响应时出现 `Broken pipe`，数据库显示润色实际已成功落库为第 2 版歌词且 `polish_used_count=1`；本质是 DeepSeek 多 Agent 同步润色耗时过长，公网/代理连接先断开，造成前端误判失败。已在 Claude Web v1 为润色/续写补短期韧性恢复：连接中断时不立即作为最终失败，而是在弹窗内提示“请求可能仍在后台处理中，正在为你确认最新歌词…”，随后短轮询作品详情；若发现歌词版本号已更新，自动刷新页面、关闭弹窗并提示已获取新歌词。`useAction` 增加可选的错误 toast 抑制能力，让长请求恢复流程使用弹窗内状态而不是全局错误提示。验证通过：`npm test -- ConfirmView.test.tsx`、`npm run typecheck`、`npm run build`、`git diff --check`。当前仍是短期前端兜底；长期更稳方案是将润色/续写改为真正异步任务，避免公网 HTTP 长连接等待 DeepSeek 完整生成。 |
| 2026-06-15 19:42 CST | 落地 AI 润色/续写异步后台任务 | 按长期方案将 `AI 润色` / `AI 续写` 从公网 HTTP 长请求改为后台任务：新增 `lyrics_edit_jobs` migration，`WorkService` 在收到润色/续写请求后只校验当前可编辑草稿、改词次数和并发任务，然后创建 `QUEUED` job 并立即返回最新作品；`LyricsEditJobDispatcher` 定时 claim due jobs，`LyricsEditJobProcessor` 在后台调用真实/Mock 写词服务，成功后用 `works.version` 和 source draft 前置条件写入新歌词草稿并标记 job `SUCCEEDED`，失败则记录 `failure_code/message/retryable`，避免长连接断开造成“前端失败、后台成功”的假失败。确认出歌在 active job 存在时返回 409，前端会展示“AI 正在润色/续写”、保留旧歌词、禁用改词与确认出歌并轮询详情；失败时展示最近一次改词失败提示，成功后自动刷新到新草稿。OpenAPI 已新增 `active_lyrics_job` 与 `last_lyrics_edit_failure` 字段。本轮验证通过：`:apps:music-api:compileJava`、`:apps:music-api:test --tests WorkServiceWorkflowDispatchTest --tests LyricsEditJobProcessorTest`、Java 回归 `:apps:music-api:test :modules:production:test :modules:lyrics:test :modules:deepseek:test spotlessCheck`、Claude Web v1 `npm test` / `typecheck` / `build`、静态 OpenAPI 契约、`git diff --check`。已重新构建并重启本地真实 API/worker，Flyway 版本到 `202606151930`；readiness 确认为 DeepSeek real + Yunwu Suno + WellAPI Image2 + S3ObjectStorageClient + album-ffmpeg + outbox/Temporal，worker 确认为 `RealDeepSeekMusicPromptAgent` / `RealDeepSeekCoverPromptAgent` / `RealDeepSeekQualityEvaluationAgent`。公网前端 `/api/v1/me` 返回 200，当前链接仍可给可信朋友继续真实测试；本轮未主动发起新的 DeepSeek/Yunwu/WellAPI 生成调用。 |
| 2026-06-15 20:45 CST | 首轮写词改为后台任务 | 针对公网用户首次“生成歌词”提示“作曲服务中断”但后台实际成功的问题，完成首轮写词异步化：`POST /works/inspiration` 和 `POST /works/lyrics` 现在只做输入校验、创建 `LYRICS_GENERATING` 作品占位、写入 `generation_jobs` 与通用歌词任务后立即返回 `work_id`；DeepSeek 写词由 `LyricsEditJobDispatcher` / `LyricsEditJobProcessor` 后台执行，成功后写入第一版 `lyrics_drafts` 并推进到 `LYRICS_READY / WAITING_CONFIRM`，失败后落 `LYRICS_FAILED` 和可读失败码。`lyrics_edit_jobs` 迁移扩展为可承接 `CREATE_INSPIRATION` / `CREATE_LYRICS` / `POLISH` / `CONTINUE` 的通用歌词任务，保留现有润色/续写能力。Claude Web v1 已补回归测试，确认作品详情在 `LYRICS_GENERATING` 时展示歌词生成中并轮询到确认页；OpenAPI smoke 动态契约改为创建后等待 `LYRICS_READY` 再继续后续润色、确认、出歌链路。当前验证已通过后端目标测试和前端目标测试；后续仍需跑完整 Java/前端回归、重启本地真实 API/worker，并在公网入口做一次不调用音乐的首轮写词验证。 |
| 2026-06-16 00:51 CST | 完成真实用户完整浏览器自动化测试 | 按用户要求不使用 Computer Use，改用 Playwright 浏览器自动化 + API/日志/媒体校验完成真实用户完整测试。测试前发现真实环境仍有旧 API/worker Java 进程占用 `8080/8081`，导致错误响应路径出现 `ThrowableProxy` 日志类加载异常并让前端误报“作曲服务连接中断”；同时 `music-worker` 在 `LYRICS_EDIT_DISPATCHER_ENABLED=true` 下缺少 `LyricsGenerationService` 依赖，说明首轮/改词后台任务还没有真正由 worker 稳定承接。已修复 `apps/music-worker` 依赖和 `WorkerProductionConfiguration`，补齐 Knowledge/Prompt/DeepSeek/Lyrics 相关 Bean，重新构建并重启 API/worker；验证 readiness 为 DeepSeek real、Yunwu Suno、WellAPI Image2、S3ObjectStorageClient、`album-ffmpeg`、API dispatcher disabled / worker expected、音乐/封面并行 `music-cover-parallel/2`。真实样本只跑 1 条，作品 `fa372c05-cdfc-4a50-93a4-a9984df76ca0`（《跑过清河》）最终达到 `GENERATED / PACKAGE_FETCHED`：首轮写词、润色、续写均由后台任务完成；音乐 `16:39:03-16:42:37`、封面 `16:39:25-16:42:27` 时间重叠，证明并行生效；视频 `14.1s`、发布包 `6.1s` 完成。浏览器验证 390px/1440px 无横向溢出，视频 metadata 可读、音频 metadata 可读、封面可见，发布包 audio/video Range GET 为 `206`、cover GET 为 `200`，刷新下载链接和标记交接通过；ffprobe 验证 MP4 为 H.264 1920x1080 + AAC，时长约 `253.24s`。Mock UI 补测确认创作首页新字段、填词成歌、歌词生成中和确认页布局可用。验证通过：OpenAPI 静态契约、真实模型安全门、Java 目标测试、Claude Web v1 `npm test/typecheck/build`、严格知识库校验、导入 dry-run、`spotlessCheck :apps:music-worker:test :apps:music-worker:bootJar` 和 `git diff --check`。残留风险：公网访问控制/次数限额仍按用户要求暂缓；公司 Adapter 仍是 Mock；本地 PostgreSQL collation warning 与已知知识库 alias warning 后续维护处理；测试报告未记录密钥、完整 Prompt、供应商原始响应或签名媒体 URL。 |
| 2026-06-16 10:01 CST | 阶段 2：收束长任务前端体验与 smoke 异步语义 | 针对首轮写词、润色、续写已后台任务化后的稳定性，本轮修复 Claude Web v1 对错误和轮询的判断：非配额 409 不再被误提示为“改词次数已用完”，只有网络/502/503/504 才进入“连接短暂中断，正在重连”；业务错误会展示可读错误而不是无限重连；作品详情在空洞态改为稳定“正在同步作品状态”，避免再次闪现“作品状态无法识别”；润色/续写请求断线后，如果后台 job 已创建，弹窗会退出到页面轮询态，不再长时间卡在弹窗里。同步修复 smoke 脚本的旧同步假设：本地商用后端 stack、DeepSeek 真实写词 stack、WellAPI/DreamMaker Image2 stack、OpenAPI 动态契约、DreamMaker guard、Temporal stepwise、Yunwu/DreamMaker 音乐 smoke 均改为启动或等待 worker，并等待 `LYRICS_READY / WAITING_CONFIRM` 或草稿版本推进后再确认/断言。验证通过：相关 smoke 脚本 `bash -n`、Claude Web v1 `npm test/typecheck/build`、`OPENAPI_CONTRACT_STATIC_ONLY=true scripts/smoke/openapi-contract.sh`、`scripts/smoke/real-model-safety-gates-audit.sh`、`git diff --check`。本轮未调用 DeepSeek/Yunwu/WellAPI/DreamMaker 真实生成。 |
| 2026-06-16 10:12 CST | 阶段 3：作词 Agent 主线与润色语义收束 | 针对用户实测歌词“意象散、听不懂在唱什么”和 AI 润色“像重写一首歌”的问题，本轮不改知识库检索、不锁死曲风模板，而是在作词 Agent 内补“歌曲主线骨架”：`CreativeBriefAgent` 新增 `song_thesis`、`pov`、`central_tension`、`emotional_turn`、`chorus_function`、`memory_device`，让 LyricsAgent 写之前明确这首歌到底在唱什么、谁在唱、核心张力和副歌功能；`LyricsAgent` Prompt 强化单一歌曲命题、视角稳定、段落服务同一主线、副歌必须承担功能，并把 `POLISH` 明确为真正润色：保留原歌歌名、视角、主旨、情绪弧线和大多数可用歌词，只按用户要求优化弱点，不默认改成另一首歌；`QualityEvaluationAgent` 新增主旨清晰度、段落推进、视角一致、副歌功能和真实润色检查。服务层会把这些字段传入生产写词 prompt 和质量门上下文，一次质量门重写也会围绕主线、视角和副歌功能定向修。验证通过：`:modules:deepseek:test --tests '*RealDeepSeekLyricsClientTest' --tests '*RealDeepSeekCreativeAgentsTest' :modules:lyrics:test --tests '*DefaultLyricsGenerationServiceTest'`、`:modules:deepseek:test :modules:lyrics:test`、`spotlessCheck`。本轮未调用真实 DeepSeek/Yunwu/WellAPI/DreamMaker。 |

| 2026-06-16 10:35 CST | 阶段 4：补齐阶段级失败恢复矩阵 | 本轮将出歌后恢复动作从“笼统联系客服”收束为真实可执行矩阵：音乐失败继续 `RETRY_MUSIC`；封面失败 `RETRY_COVER` 复用已成功音频，只补封面、视频和发布包；视频失败 `RERENDER_VIDEO` 复用音频和封面，只重渲视频并重建发布包；发布包失败新增 `REBUILD_PACKAGE`，复用音频、封面和视频，只重建发布素材。`SongProductionWorkflowInput` 新增 `reuseExistingCover` / `reuseExistingVideo`，生产工作流会按恢复类型跳过不应重复调用的音乐、封面或视频供应商；API 新增 `POST /api/v1/works/{work_id}/publish-package/rebuild`，OpenAPI 和 Claude Web v1 `AvailableAction` 已同步。验证通过：`:modules:work-domain:test --tests *WorkStateMachineTest`、`:apps:music-api:test --tests *WorkServiceWorkflowDispatchTest`、生产工作流复用音频/封面/视频目标测试、Claude Web v1 `npm test -- FailedView` 与 `typecheck`、OpenAPI 静态契约、`spotlessCheck` 和 `git diff --check`。本轮未调用真实 DeepSeek/Yunwu/WellAPI/DreamMaker。 |
| 2026-06-16 11:20 CST | 阶段 5：真实测试回归与长任务稳定收口 | 本轮真实完整回归发现并修复 4 个长期稳定性问题：`music-worker` 未绑定 `yanyun.knowledge` 配置导致真实 worker 没有按 `KNOWLEDGE_RETRIEVAL_MODE=pgvector` 使用知识库；公网完整体验脚本等待首轮歌词的上限仍按旧同步链路设置，DeepSeek 慢响应时会误判超时；`bootRun` 退出后可能残留子 Java 进程占用 `8080/8081`，导致下一次测试打到旧进程；`LyricsEditJobDispatcher` 以同步方式处理任务，旧的慢任务或 stale `RUNNING` job 会阻塞后续新歌词任务。已补齐 worker 知识库配置与启动日志、将真实 smoke 歌词等待改为可配置、清理脚本增加端口 listener 兜底停止，并把歌词任务 dispatcher 改成按 batch size 控制的有界异步执行器，同时优先 claim `QUEUED` 新任务，避免旧任务饿死新任务。真实完整样本 `994165e2-59e4-4db4-bc7d-e79ca9f01bf7` 已跑通 `DeepSeek v4Pro + pgvector KB + Yunwu Suno chirp-fenix + WellAPI Image2 + album-ffmpeg + S3/MinIO`，最终 `GENERATED / PACKAGE_READY`，audio/cover/video/timeline 均存在，发布素材可读，MP4 通过 H.264 + AAC 双轨校验，前端成品页刷新链接和标记交接通过。阶段耗时证据显示音乐 `143.8s`、封面 `207.3s` 且时间重叠，视频 `9.0s`、发布包 `5.4s`，确认音乐/封面并行真实生效。验证通过：Java `:apps:music-api:test :apps:music-worker:test :modules:production:test :modules:knowledge:test spotlessCheck`、Claude Web v1 `npm test/typecheck/build`、严格知识库校验与导入 dry-run、OpenAPI 静态契约、真实模型安全门审计、真实完整体验 stack smoke 和 `git diff --check`。残留风险：公网访问控制/次数限额按用户要求暂缓；公司 Adapter 仍为 Mock；DeepSeek/CoverPrompt 真实响应仍可能较慢；本地 PostgreSQL collation warning 与少量知识库 canonical alias warning 后续维护处理。 |
| 2026-06-16 14:03 CST | 修复燕云实体模糊纠错误拦截 | 用户公网测试首轮写词失败，前端显示“歌词不够贴合燕云十六声”。排查最新失败作品 `f8ccc78f-0c30-4d7b-8e62-131c45d1346c` 后确认根因不是歌词完全跑题，而是 `KnowledgeEntityResolver` 将用户输入中的“十六州”按编辑距离误匹配为玩法概念“十六声”，`QualityEvaluationAgent` 又把该 fuzzy match 当成“错字名被输出”而触发 `LYRICS_QUALITY_FAILED`。已将知识库模糊纠错白名单收紧为人物、剧情/任务线、地域/地点、势力/门派；玩法概念、创作边界等只允许精确 alias 命中，质量门的“错字名输出”本地兜底也使用同一白名单，避免把“十六州”这类合法世界词误判成“十六声”的错字。新增回归测试覆盖：`十六州` 不再 fuzzy 命中 `gameplay/寻声/十六声`，但精确 `十六声` 仍可命中；质量门收到 `gameplay/寻声 matched=十六州 kind=fuzzy` 时不再拒绝，同时保留人物错字纠正后的输出拦截。验证通过：`:modules:knowledge:test --tests '*KnowledgeEntityResolverTest'`、`:modules:deepseek:test --tests '*RealDeepSeekCreativeAgentsTest.qualityAgentLocalSafetyDoesNotRejectGameplayFuzzyWorldTerm' --tests '*RealDeepSeekCreativeAgentsTest.qualityAgentLocalSafetyRejectsCorrectedEntityTypoInLyrics'`、`:modules:knowledge:test :modules:deepseek:test --tests '*RealDeepSeekCreativeAgentsTest'`、严格知识库校验、导入 dry-run、`spotlessCheck`、`git diff --check`。残留：该失败作品保持失败状态，未直接改数据库假装成功；重启真实 API/worker 后，新建或重新发起写词会使用修复后的纠错逻辑。 |
| 2026-06-16 16:10 CST | 修复 Yunwu 占位音频 URL 导致的收尾误报 | 用户公网测试作品 `d83d2741-5d98-4834-aeef-3307d5d02343`（《弦上州》）失败根因确认：Yunwu/Suno 任务本身返回成功，但 fetch 结果里的音频地址为供应商占位/不可下载地址，Range GET 返回 HTTP 403，旧逻辑先把它当作可用音频进入后续收尾，最终误报为 `PACKAGE_BUILD_FAILED` / “作品收尾时出了点问题”。本轮新增远程媒体导入异常分类与音频重新获取接口：HTTP 401/403/404/429/5xx 会归类为 `AUDIO_IMPORT_FAILED` 并可走 `RETRY_AUDIO_IMPORT`；同时补 legacy 兼容，旧的 `PACKAGE_BUILD_FAILED` 且缺 AUDIO 的作品也能进入音频获取重试。进一步收紧 Yunwu fetch 解析：`None.mp3` / 空音频等占位 URL 不再视为成功音频；若供应商任务没有可用音频，最终归类为 `MUSIC_GENERATION_FAILED`，下发 `RETRY_MUSIC`，避免用户反复重试“收尾”却永远拿不到音频。已重启公网真实 API/worker；对旧作品执行一次受控音频获取重试后，状态正确落为 `FAILED / MUSIC_GENERATION_FAILED`，可执行动作变为 `RETRY_MUSIC` 和 `RETURN_TO_EDIT`，未重新调用 Suno 生成新音乐。验证通过：`:apps:music-api:test :modules:production:test :modules:storage:test :modules:suno:test :modules:work-domain:test`、`:modules:suno:test` 定向 parser 测试、相关 WorkService/Workflow 定向测试、`spotlessCheck`、API/worker `bootJar`、`git diff --check`，公网 readiness 仍为 DeepSeek real + Yunwu Suno + WellAPI Image2 + S3ObjectStorageClient + album-ffmpeg + 音乐/封面并行。 |

| 2026-06-16 17:08 CST | 收束 AI 润色/续写慢失败体验 | 用户公网测试暴露 AI 润色耗时约 6 分 45 秒后失败，根因为 POLISH 仍走完整写词链路：KnowledgeRetrieve 后连续调用 CreativeBriefAgent、LyricsAgent、QualityEvaluationAgent，并在 DeepSeek 空响应/坏 JSON 时叠加多轮语义重试和 job 级重试。已将 POLISH/CONTINUE 改为轻量编辑链路：保留知识库和实体解析，但跳过 CreativeBriefAgent 与完整质量门，只调用 LyricsAgent 一次编辑并做本地边界检查；DeepSeek 编辑类语义重试从 3 次降为 2 次，请求超时最多 120 秒，编辑 job 最大尝试次数降为 1，避免用户等待数分钟后才失败。前端 active job 文案加入“通常 30-90 秒”，超过 90 秒展示“仍在后台处理中，原歌词会保留”的慢提示。验证通过：`JAVA_HOME=/opt/homebrew/opt/openjdk@21 ./gradlew --no-daemon :apps:music-api:test :apps:music-worker:test :modules:lyrics:test :modules:deepseek:test :modules:production:test spotlessCheck`、Claude Web v1 `npm test -- ConfirmView.test.tsx` / `npm run typecheck` / `npm run build`、`git diff --check`。本轮未调用 Yunwu、WellAPI、DreamMaker，也未生成新音乐/封面/视频；修复后仍需重启真实 API/worker 才对公网入口生效。 |
| 2026-06-16 17:42 CST | 修复 Yunwu 轮询异常丢失任务号与错误重试扣费风险 | 用户实测作品 `bd8a3617-66c6-4dd2-95b4-fc5849e50023` 两次“重新生成”并非同一失败：第一次在 `MusicPromptAgent` 即失败，未调用 Yunwu；第二次已成功提交到 Yunwu 并很可能产生扣费，但轮询 `/suno/fetch/{taskId}` 时遇到供应商 502，旧实现把已拿到的 `taskId` 丢成 `null`，平台只能把这次任务记为 `MUSIC_GENERATION_FAILED`，后续重试又会重新发起生成，存在重复扣费风险。已修复 `YunwuSunoMusicProvider`：提交成功后若轮询异常，失败结果仍保留 `providerTaskId`；同时收束 `retryMusic` / `retryAudioImport` 恢复链路，若作品无 AUDIO 且存在最近一次 Yunwu `provider_trace_id`，则优先走 `audioImportProviderTaskId` 恢复已有供应商任务，复用已生成封面而不是直接重新调用 Suno。新增回归测试覆盖“submit 成功 + fetch 502 仍保留 taskId”及“retryMusic 优先复用 recoverable task id”。验证通过：`JAVA_HOME=/opt/homebrew/opt/openjdk@21 ./gradlew --no-daemon :modules:suno:test --tests '*YunwuSunoMusicProviderTest'`、`JAVA_HOME=/opt/homebrew/opt/openjdk@21 ./gradlew --no-daemon :apps:music-api:test --tests '*WorkServiceWorkflowDispatchTest'`、`git diff --check`。当前这次已失败作品因历史记录中未保存 `provider_trace_id`，无法自动救回；修复已防止后续同类真实测试再次“供应商已扣费但平台记成彻底失败”。 |
| 2026-06-16 18:20 CST | 启动作词 Agent v0.8 金曲奖级调优闭环 | 按用户确认的“Prompt + 歌词联评、由 Codex 严苛担任评委”方向，完成 v0.8 第一阶段工程落点：`CreativeBriefAgent` 从 v0.7 升级到 v0.8，新增 `song_core`、`singer_voice`、`listener_target`、`emotional_engine`、`chorus_job`、`avoid_direction` 六个歌曲入口判断字段，避免把一句话玩家灵感放大成剧情大纲；`LyricsAgent` Prompt 增加“先完成一首歌，再服务规则”的写作优先级，要求知识库只服务歌曲主线，不摊成资料清单；`QualityEvaluationAgent` 同步到 v0.8 评审口径，强调结合用户输入、Prompt、知识库、Brief 和歌词一起看。新增 24 条真实玩家一句话输入评测集 `knowledge-base/lyrics-agent-v0.8-eval-cases.json`、A/B 评测脚本 `scripts/knowledge/run-yanyun-lyrics-agent-v08-ab.py` 和规格文档 `docs/specs/lyrics-agent-v0.8-golden-lyrics-tuning.md`；完整 Prompt 与完整歌词只会输出到 ignored 的 `build/reports/lyrics-agent/`，不得提交。当前尚未调用真实 DeepSeek 生成 A/B 样本，下一步是先跑 6 条小样本并由 Codex 按金曲奖级评分表逐首联评。 |
| 2026-06-16 23:13 CST | 完成作词 Agent v0.8 首批 A/B 严格评审与 v0.8.1 微调 | 已执行 6 条真实玩家输入的 DeepSeek 文本 A/B，完整 Prompt 与歌词仅保存在 ignored 的 `build/reports/lyrics-agent/`，提交区只保留脱敏评审摘要 `knowledge-base/lyrics-agent-v0.8-ab-review-summary.json`。评审结论：v0.8 初版只赢 1/6，不进入 24 条扩大评测；主要问题不是燕云归属感不足，而是 `song_core` 被直接唱成显眼口号，削弱了真实细节、暧昧空间和人物质感。已将 Prompt 调整到 v0.8.1：要求 `song_core` 作为暗中统领，不直接反复喊主题句；副歌通过具体声音、动作、物件、句式变奏或意象回环完成 `chorus_job`；每首歌至少保留一个来自用户输入或知识库的真实细节；粗粝题材避免空喊、泛化反叛和无必要粗口。下一步建议用同一 6 条样本复跑 v0.8.1，对比 v0.7 和 v0.8 初版是否真正改善。 |
| 2026-06-16 23:54 CST | 完成作词 Agent v0.8.1 复评并推进 v0.8.2 | 已复跑同一批 6 条真实玩家输入，完整 Prompt 与歌词继续只保存在 ignored 的 `build/reports/lyrics-agent/`，提交区新增脱敏摘要 `knowledge-base/lyrics-agent-v0.8.1-ab-review-summary.json`。评审结论：v0.8.1 胜出 3/6，较 v0.8 初版明显改善，但仍未达到扩大 24 条的门槛；胜出在寒香寻等待、市井烟火、九流门粗粝题材，失败集中在清河地点怀念、小人物出手、轻快后劲酸。已继续调到 v0.8.2：地点怀念类要敢少写，只选一个小感官仪式或重复物件反复变义；小人物题材保留笨拙温暖和人间尺度，避免拔高成英雄受难；轻快转酸楚优先使用共同口头禅、玩笑、动作或小约定，不默认写成泛化想念。下一步建议只复跑这 3 个失败题，若 v0.8.2 至少赢 2/3，再恢复 6 条完整批评测。 |
| 2026-06-17 00:35 CST | 完成作词 Agent v0.8.2 复评并推进 v0.8.3 | 已先复跑 v0.8.2 的 3 个失败题，结果胜出 2/3：小人物出手和轻快后劲酸改善明显，清河地点怀念仍输给 v0.7；随后恢复完整 6 条回归，结果 v0.8.2 只胜出 1/6，说明 Prompt 仍偏“纠正型”和“规则型”，会把模型推向安全、平整、普通的歌词。完整 Prompt 与歌词仍只保存在 ignored 的 `build/reports/lyrics-agent/`，提交区新增脱敏摘要 `knowledge-base/lyrics-agent-v0.8.2-targeted-review-summary.json` 与 `knowledge-base/lyrics-agent-v0.8.2-full6-review-summary.json`。已继续调到 v0.8.3：不再累加规则，改为更短的作词优先级：一个私人声音装置、一个具体情绪发动机、一个副歌变义方式；如果用户输入、知识库或 Brief 里已有“笨啊你”“再喝一碗”“茶碗一响”这类更鲜活的装置，优先保留并深化。下一步建议只用清河、小人物、轻快后劲酸 3 条复跑 v0.8.3，验证是否不再把强装置替换成普通主题句。 |
| 2026-06-17 01:22 CST | 完成作词 Agent v0.8.3 targeted 复评并补 v0.8.4 边界修复 | 已复跑清河地点怀念、小人物出手、轻快后劲酸 3 条 targeted 样本，完整 Prompt 与歌词继续只保存在 ignored 的 `build/reports/lyrics-agent/`，提交区新增脱敏摘要 `knowledge-base/lyrics-agent-v0.8.3-targeted-review-summary.json`。v0.8.3 胜出 2/3：清河样本“茶还热着吗”更私人、更少铺陈；轻快后劲酸样本“走啊，去看看”成为目前最接近目标的私人口头禅式 hook；但小人物出手样本误把摇滚能量写成“拳套、沙袋”等现代拳击场景，破坏燕云归属感。已补 v0.8.4：曲风只影响节奏、能量、声口和句子颗粒度，不允许自动带入现代道具或现代场景；小人物/少侠题材必须落回旧剑、木棍、泥路、茶摊、驿站、斗笠、雨、药布、护送、挡一下等燕云可成立的生活材料。单条复测显示现代道具漂移已修复，但副歌仍偏普通，提交区新增 `knowledge-base/lyrics-agent-v0.8.4-underdog-review-summary.json`；当前判断 v0.8.4 是边界修复，不是金曲级质量突破，暂不扩大 24 条。下一步建议不要继续无限堆 Prompt 规则，而是评估是否引入轻量候选选择机制，或针对“小人物”增加更私人、非口号的弱点标记策略。 |
| 2026-06-17 02:05 CST | 完成作词 Agent v0.8.4 六条回归评审 | 已复跑首批 6 条真实玩家输入，完整 Prompt 与完整歌词继续只保存在 ignored 的 `build/reports/lyrics-agent/`，提交区新增脱敏摘要 `knowledge-base/lyrics-agent-v0.8.4-full6-review-summary.json`。严格评审结果：v0.8.4 胜出 3/6，未达到扩大 24 条门槛；胜出在小人物出手、市井烟火、轻快后劲酸，失败在寒香寻、清河、九流门。v0.8.4 证明“曲风不得带入现代道具”的边界修复有效，也能产出“走吧/跑调旧歌”“一二三倒数”等较好的私人声音装置；但在寒香寻和清河上偏文学化，在九流门上过度清洁，说明 Prompt-only v0.8 线已接近边际收益上限。下一步建议二选一：若允许增加一次低成本文本调用，做轻量候选选择机制（先选 2 个副歌装置/进入角度，再写完整歌词）；若必须保持单调用，则回退成极简 Prompt，只保留 song_core、私人声音装置、燕云边界、可唱性和质量自检，删除大量题材专门规则。 |
| 2026-06-17 16:16 CST | 完成作词 Agent v0.9.2 六条压力测试 | v0.9.2 聚焦修复 CraftPlan 第一判断：当用户原句本身已经像 hook（例如“还是会出手”）时，必须保留原句或近似原句，不再替换成更电影感但更不适合唱的新装置。已先跑 v0.9.1 输掉的 4 条真实 DeepSeek 文本样本，再补跑初始集剩余 2 条，完整 Prompt 与完整歌词继续只保存在 ignored 的 `build/reports/lyrics-agent/`。脱敏摘要已新增 `knowledge-base/lyrics-agent-v0.9.2-full6-review-summary.json`。严格评审结论：v0.9.2 胜出 `4/6`，达到进入 12 条扩大评测的最低门槛；但它在“轻快但难受”和“反救世主，只救眼前人”两类输入仍输给 v0.7，且真实调用耗时显著增加（B 组 total P50 约 132.8s/145.0s，对比 A 组约 80.8s/63.2s）。当前判断：v0.9.2 值得进入 12 条扩大评测，但不能直接作为生产默认 Prompt；下一步应评估 12 条压力集，若不足 `8/12` 胜出，就停止自然语言 Prompt 叠加，改为结构化 CraftPlan 决策。 |
| 2026-06-17 16:55 CST | 完成作词 Agent v0.9.2 十二条压力测试并停止扩样 | 已执行 12 条 v0.9.1 压力集的真实 DeepSeek 文本 A/B，完整 Prompt 与完整歌词继续只保存在 ignored 的 `build/reports/lyrics-agent/`，提交区新增脱敏摘要 `knowledge-base/lyrics-agent-v0.9.2-mid12-review-summary.json`。严格评审结论：v0.9.2 只胜出 `5/12`，v0.7 胜出 `7/12`，未达到 `8/12` 扩大门槛；v0.9.2 在强用户原句、寒姨角色反等待、小摊贩普通人、跑图朋友退坑、底层江湖噪音上更好，但 v0.7 在轻快后劲酸、反宏大守护、失败者尊严、R&B 夜酒、小铃声、摇滚普通少侠等题上更自然、更直接或更像歌。耗时方面，A 组 total P50 约 `76.6s`，B 组 total P50 约 `143.9s`，CraftPlan 自身 P50 约 `33.9s`，质量收益不足以支撑默认多一次调用。当前决策：不扩 24 条，不将 v0.9.2 作为生产默认 Prompt，不继续堆自然语言规则；下一版若继续 planner 路线，应改为结构化 CraftPlan，显式输出 `user_phrase_assessment`、`lyric_surface_mode`、`device_decision` 和 `latency_budget`，并区分玩家元叙事与燕云世界内叙事。 |
| 2026-06-17 18:10 CST | 实现作词 Agent v0.10 结构化 CraftPlan 决策 | 按 v0.9.2 复盘结论，已将 CraftPlan 从自然语言“写法导演”改为结构化分诊：新增 `planning_decision`、`user_phrase_assessment`、`lyric_surface_mode`、`device_decision`、`latency_budget`、`confidence`，允许 `SKIP_PLAN`、`NOT_WORTH_EXTRA_CALL` 或低置信时走直写，避免所有首轮灵感都强制多一次 DeepSeek 规划。`LyricsAgent` system prompt 已从 v0.8/v0.9 长规则块收束为 v0.10 短契约：先完成一首能被听懂、能被记住、值得被唱的歌；只有 `USE_PLAN` 时才使用 selected fields；区分 `IN_WORLD`、`PLAYER_META`、`CHARACTER_SONG`、`ORDINARY_STORY` 表层模式。A/B 脚本默认 current variant 改为 `B_v0.10`，并按生产逻辑处理 `SKIP_PLAN`。规格文档已同步。待验证：Java 定向测试、spotless、脚本 dry-run；待评审：重新跑 12 条 DeepSeek 文本 A/B，至少 `8/12` 胜出才允许扩大到 24 条或进入真实产品默认路径。 |
| 2026-06-17 19:05 CST | 验证作词 Agent v0.10 并补 v0.10.1 CraftPlan 本地 pre-gate | 已提交 v0.10 工程快照 `845e14a4 feat: add lyrics craft planner v0.10 decision gate`，随后用双 gate 执行 6 条真实 DeepSeek 文本 A/B。第一轮发现 v0.10 大约 `4/6` 胜出，但 6 条中只有 1 条真正使用 CraftPlan，其余 5 条是模型规划后判定 `SKIP_PLAN / NOT_WORTH_EXTRA_CALL`，说明“多一次规划调用”多数时候没有贡献。已补 v0.10.1 本地 pre-gate：只在用户输入含玩家元表达（如“新手村、跑图、退坑、我的角色”等）或明显含糊需要找入口时调用 CraftPlan；强歌核输入直接写，生产链路和 A/B 脚本保持一致。复跑 6 条后，CraftPlan 只在清河“新手村”样本触发，工程行为符合预期，但严格人工评审约为 `3/6`，未达到扩大到 12 条的门槛；B 组仍受单次 DeepSeek 写词慢响应拖累，且质量收益不稳定。当前判断：v0.10.1 的 pre-gate 值得保留以降低无效规划调用，但“单次写词 Prompt + 可选 planner”已经接近收益上限；下一步不应继续堆自然语言规则，应规划多候选生成/轻评审选择或回退更短直写 Prompt。完整 Prompt 与完整歌词仍只保存在 ignored 的 `build/reports/lyrics-agent/`，未提交。 |
| 2026-06-17 20:04 CST | 作词 Agent v0.11 回退为生产直写链路 | 基于 v0.10.1 复盘结论，已从生产写词链路移除 CraftPlan：删除 `LyricsCraftPlanner` 接口、Mock/Real 实现、API/worker Bean、`LYRICS_CRAFT_PLANNER_ENABLED` 配置和 A/B 脚本中的 CraftPlan 调用；首轮写词恢复为 `KnowledgeRetrieve -> CreativeBriefAgent -> LyricsAgent -> QualityEvaluationAgent`。`RealDeepSeekLyricsClient` 更新为 v0.11 direct-write Prompt：直接从用户输入、CreativeBrief、resolved entities 和知识库上下文选择清楚歌曲主线，不再等待额外规划步骤。A/B 脚本默认 current variant 改为 `B_v0.11-direct`，报告不再包含 CraftPlan 段落或耗时。本轮未调用真实 DeepSeek/Yunwu/WellAPI/DreamMaker，未生成音乐/封面/视频。已通过：`python3 -m py_compile scripts/knowledge/run-yanyun-lyrics-agent-v08-ab.py`、`:modules:creative-agent:test :modules:deepseek:test :modules:lyrics:test`、`:apps:music-api:test :apps:music-worker:test`。下一步建议跑 v0.11 direct 的 6 条文本 A/B，确认质量至少不弱于 v0.7/v0.10.1 后再考虑公网真实测试。 |
| 2026-06-17 20:30 CST | 完成作词 Agent v0.11 六条真实文本 A/B | 已提交 v0.11 生产直写链路快照 `22166602 feat: remove lyrics craft planner from production flow`，并用双 gate 执行 6 条真实 DeepSeek 文本 A/B；完整 Prompt 与歌词继续只保存在 ignored 的 `build/reports/lyrics-agent/`，不进入 Git。结果：v0.11 相比 v0.7 在九流门粗粝题材、市井烟火、轻快转酸楚样本上更像歌，尤其能保留“铜板响三声”“把灯再拨亮一些”“说好不回头”这类声音装置；寒香寻和清河怀旧题仍是 v0.7 更完整、更稳，v0.11 有时更私人但也更窄、更散。小人物出手样本两边都可用，v0.7 更直接，v0.11 更规整但略普通。耗时方面，v0.11 不再多一次 CraftPlan，但本轮 DeepSeek 真实响应仍慢于 v0.7：A 组 total P50 约 56.5s，B 组 total P50 约 92.3s，主要来自 brief/lyrics 两次真实文本调用波动。当前判断：删除 CraftPlan 是正确的长期方向，v0.11 可以作为比 v0.10 更简单的生产基线继续打磨；但它尚未达到“金曲奖级稳定提升”，不建议继续堆大段 Prompt 规则，下一步应收束为更短、更稳定的直写 Prompt，并重点修“抒情怀旧题容易变散/变窄”的通用判断。 |
| 2026-06-17 21:03 CST | 验证作词 Agent v0.11.1 短 Prompt 实验未达标 | 在 v0.11 生产直写基础上尝试 `B_v0.11.1-short`：把 LyricsAgent system prompt 从长规则清单收束为“歌核、声音、记忆装置、副歌任务、燕云落点”短契约，并同步 A/B 脚本变体名。验证通过：`:modules:deepseek:test --tests '*RealDeepSeekLyricsClientTest'`、`:modules:deepseek:test :modules:lyrics:test`、`spotlessCheck`、脚本 `MODE=plan/preflight`、`git diff --check`。随后执行 6 条真实 DeepSeek 文本 A/B，完整 Prompt 与歌词仍只保存在 ignored 的 `build/reports/lyrics-agent/`。结果：v0.11.1 明显降低本轮 B 组耗时（total P50 约 54.9s，对比 A 组约 69.0s，也优于上轮 v0.11 的 92.3s），但质量约只胜出 2/6；优势在小人物“再试一次”和轻快转酸楚“没事啊”，失败在寒香寻、清河、市井、九流门：短 Prompt 容易把抒情怀旧写成普通问句，把粗粝题材写成“爷就是规矩”式硬口号。当前判断：v0.11.1 不建议作为生产默认 Prompt 合入；它证明“更短更快”有价值，但需要保留 v0.11 对完整情绪弧线、人味和反口号的约束。下一步建议回到 v0.11 作为生产基线，另开 v0.11.2 做极少量补强，或直接进入多候选/人工挑选路线。 |
| 2026-06-17 21:51 CST | 保留作词 Agent v0.11.2 最小补强 | 已先将 v0.11.1 失败实验文档提交为 `7080d9f2 docs: record lyrics v0.11.1 prompt experiment`，随后回到 v0.11 direct-write 基线，只做 v0.11.2 极小补强：抒情/怀旧/角色歌的物件必须推动关系、时间或缺席变化；粗粝/底层/失败者题材用具体活法替代“爷、规矩、命、热血”式硬口号。验证通过：`:modules:deepseek:test --tests '*RealDeepSeekLyricsClientTest'`、`:modules:deepseek:test :modules:lyrics:test`、`spotlessCheck`、脚本 `MODE=plan/preflight`、`git diff --check`。已执行 6 条真实 DeepSeek 文本 A/B，完整 Prompt 与完整歌词继续只保存在 ignored 的 `build/reports/lyrics-agent/`，提交区新增脱敏摘要 `knowledge-base/lyrics-agent-v0.11.2-full6-review-summary.json`。严格评审约为 v0.11.2 胜出 4/6：寒香寻、小人物出手、轻快后劲酸、九流门更好；清河和市井烟火气仍是 v0.7 更自然。耗时方面 A 组 total P50 约 61.3s，B 组约 66.2s，B 组有一次 DeepSeek 慢响应拉到约 160.6s。当前决策：保留 v0.11.2 作为当前生产直写链路的最小增量基线，但不扩大到 24 条，也不宣称金曲级质变；下一步不建议继续堆单类型 Prompt 规则，应评估小规模多候选生成/轻评审选择或人工精选歌词范例库。 |
| 2026-06-17 22:18 CST | 增加作词质量门轻量模式开关 | 按“先稳定 v0.11.2，再评估 Agent 是否过重”的判断，新增 `LYRICS_QUALITY_GATE_MODE=strict|self-score-only`。默认仍为 `strict`，保持当前正式链路：KnowledgeRetrieve、CreativeBriefAgent、LyricsAgent、QualityEvaluationAgent，低质时最多一次受控重写；新增 `self-score-only` 实验快路径会跳过额外 QualityEvaluationAgent 调用，只使用 LyricsAgent 自评分和本地安全边界检查，低自评分、其他 IP 漂移、实体错字输出仍会阻断。该开关已接入 API/worker 配置和 `.env.example`，并补充 runbook 与作词调优规格；现有公网真实测试默认不受影响。验证通过：`:modules:lyrics:test`、`:apps:music-api:compileJava`、`:apps:music-worker:compileJava`、`spotlessCheck`。下一步建议用同一批真实玩家输入对比 `strict` 与 `self-score-only` 的耗时、通过率和人工歌词质量，再决定是否把轻量模式用于默认用户路径。 |
| 2026-06-18 00:34 CST | 重启公网真实测试环境并验证 v0.12 首轮写词链路 | 已用 `public-real-test-stack.sh` 重启 API/worker 到当前 v0.12 代码，真实测试配置为 DeepSeek real、Yunwu Suno、WellAPI Image2、S3/MinIO、`album-ffmpeg`、`KNOWLEDGE_RETRIEVAL_MODE=pgvector`、`LYRICS_FIRST_DRAFT_MODE=conditional-brief`、`LYRICS_QUALITY_GATE_MODE=self-score-only`。启动前发现缺少 `S3_PUBLIC_ENDPOINT` 会 fail-fast，已使用当前媒体 tunnel 重新启动并验证 readiness：音乐/封面并行、worker 侧歌词任务、S3 存储和真实 Provider 均为 READY。随后执行 1 条真实首轮写词验证（不出歌、不生图、不渲染视频）：API 创建作品耗时约 131ms，作品 `88e8eeb4-265b-4d3f-8c18-e7b4cfad1a15` 在约 28.5s 后进入 `LYRICS_READY / WAITING_CONFIRM`，标题为《回清河》。`agent_runs` 证据显示本次只执行 `KnowledgeRetrieve` 86ms 和 `LyricsAgent` 26548ms，没有 CreativeBrief，也没有 strict QualityEvaluation 阻塞，证明当前环境已吃到 v0.12 轻量链路。当前公网前端入口为 Cloudflare 临时 tunnel，媒体根路径访问 MinIO 会返回 400 XML 属正常现象，实际签名素材 URL 需在作品生成后验证。剩余风险：尚未跑 v0.12 完整出歌样本；PostgreSQL 本地容器存在 collation version mismatch 告警，当前不影响测试但后续可维护处理。 |
| 2026-06-18 00:45 CST | 修复公网 quick tunnel 换域名后的 Vite Host 拦截 | 检查公网入口时发现旧 Cloudflare web quick tunnel 域名已出现 `SSL_ERROR_SYSCALL`，重启前端 tunnel 后获得新地址 `https://drug-lean-vendors-ranking.trycloudflare.com/`，但 Vite 返回 `Blocked request. This host ... is not allowed`。根因是 `prototypes/Claude-web-v1/vite.config.ts` 只支持单个 `VITE_ALLOWED_HOST`，quick tunnel 换域名后未被白名单允许。已将前端原型配置改为默认允许 `.trycloudflare.com`，并支持 `VITE_ALLOWED_HOSTS=host1,host2` 追加白名单；同步更新 `prototypes/Claude-web-v1/README.md`。已重启 5275 前端 dev server，验证新公网前端 `/` 返回 200、`/api/v1/me` 返回 200 且代理到真实后端正常，Claude Web v1 `npm run typecheck` 通过。当前公网前端测试链接为 `https://drug-lean-vendors-ranking.trycloudflare.com/`；媒体 tunnel 仍为 `https://compute-limousines-babies-moses.trycloudflare.com`。 |
