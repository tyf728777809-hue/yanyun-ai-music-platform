...
| `public-real-full-experience` | public-network-full-experience | `succeeded_public_smoke` | 2026-06-14 v0.6.1 knowledge-backed combined sample with DeepSeek + Yunwu Suno `chirp-fenix` + WellAPI Image 2 + `album-ffmpeg` reached `GENERATED / PACKAGE_READY`, then `PACKAGE_FETCHED`; Claude Web v1 handoff smoke verified against the same work. |
...
| 2026-06-14 | `public-real-full-experience` | DeepSeek v4Pro Agent v0.5 + pgvector KB + Yunwu Suno `chirp-fenix` + WellAPI `gpt-image-2` + album-ffmpeg MP4 | Local public network, local pgvector knowledge retrieval enabled | `bba0c628-641d-4aac-80de-110cdb8a75e7` | `<present>` | `<present>` | `GENERATED / PACKAGE_READY`, then `PACKAGE_FETCHED` after direct handoff endpoint and Claude Web v1 fixed-state verification | N/A | N/A | Audio, cover, video, timeline present in platform storage; MP4 verified H.264/AAC, 1920x1080; publish package media URLs readable | Yes | `agent_runs`: KnowledgeRetrieve used `yanyun-commercial-kb-2026-06-13-v1`; CreativeBrief, Lyrics, MusicPrompt, CoverPrompt, and QualityEvaluation used `deepseek-v4-pro`; `provider_calls`: Yunwu Suno `yunwu:suno:chirp-fenix` succeeded; `media_assets`: audio/cover/video/timeline present. Initial Claude Web v1 check exposed a local mock user id validation mismatch; after fixing `mock_user_001` handling, Playwright verified finished page, media links, handoff state, and 390px/1440px no overflow. | One controlled knowledge-backed full sample; no automatic music retry | Knowledge-backed public full path is ready for another manual user trial. Keep DreamMaker as production-target path and company systems mocked until company integration. |
| 2026-06-14 | `public-real-full-experience` | DeepSeek v4Pro Agent v0.6.1 + pgvector KB + Yunwu Suno `chirp-fenix` + WellAPI `gpt-image-2` + album-ffmpeg MP4 | Local public network, local pgvector knowledge retrieval enabled | `f334adfc-155c-4595-95d3-6f2dfc1d1fd9` | `<present>` | `<present>` | `GENERATED / PACKAGE_READY`, then `PACKAGE_FETCHED` after Claude Web v1 handoff action | N/A | N/A | Audio, cover, video, timeline present in platform storage; cover imported as `2048x1152` JPEG; MP4 verified H.264/AAC, 1920x1080; publish package media URLs readable | Yes | `agent_runs`: KnowledgeRetrieve used `yanyun-commercial-kb-2026-06-13-v1`; CreativeBrief, Lyrics, Lyrics quality gate, MusicPrompt, music quality gate, CoverPrompt, cover quality gate, and package quality gate used `deepseek-v4-pro`; `provider_calls`: Yunwu Suno `yunwu:suno:chirp-fenix` succeeded; Claude Web v1 verified finished page, media links, refresh link action, mark-fetched action, and fetched state. | One controlled v0.6.1 combined sample; no timestamped-lyrics check in this run | v0.6.1 knowledge-backed public full path is ready for user manual trial. Keep DreamMaker as production-target path and company systems mocked until company integration. |
