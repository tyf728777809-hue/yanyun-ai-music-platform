...
The project now has controlled real smoke scripts for Yunwu Suno, WellAPI Image 2, DreamMaker music, and guard validation. Those scripts either call real providers or require a running API. Before using them, the user needs a safe command that checks local shell configuration and optional API readiness without starting services or making supplier requests.
...
- FR-3: The preflight MUST support `TARGET=all|yunwu-suno|dreammaker-suno|dreammaker-minimax|deepseek|wellapi-image2|dreammaker-image2`.
