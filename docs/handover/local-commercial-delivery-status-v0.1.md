...
3. 若要测真实写词，先运行 `TARGET=deepseek MODE=plan scripts/smoke/real-model-controlled-smoke.sh` 和 `TARGET=deepseek MODE=preflight scripts/smoke/real-model-controlled-smoke.sh`，再单独打开 DeepSeek 双开关跑一条样本，不同时打开真实音乐和真实 Image 2。
4. 若要测真实封面，先运行 `TARGET=wellapi-image2 MODE=plan scripts/smoke/real-model-controlled-smoke.sh` 和 `TARGET=wellapi-image2 MODE=preflight scripts/smoke/real-model-controlled-smoke.sh`，再跑 WellAPI Image2 单作品受控 smoke；生产路径继续保留并验证 DreamMaker Image 2。
