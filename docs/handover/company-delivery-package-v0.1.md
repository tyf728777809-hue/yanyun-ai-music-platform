...
生产或公司内网部署应以 `deploy/env.production.example` 为变量名样例，启用
`SPRING_PROFILES_ACTIVE=prod`，并默认设置 `SUNO_BACKEND=dreammaker` 与
`IMAGE2_BACKEND=dreammaker`。`.env.example` 只是本地 Mock / 公网 smoke 便利配置，不能直接作为生产默认。
...
| 9 | `docs/handover/stepwise-production-implementation-task-package-v0.1.md` | 分步生产链路后续实现任务包：`stepwise-recording` 只能录步，`stepwise-production` 未实现前不能当生产链路。 |
...
`scripts/smoke/stepwise-production-boundary-audit.sh` 只做静态边界检查，不启动 API 或 worker。公司交付验收、用户实测和生产部署 smoke 必须显式保持 `TEMPORAL_SONG_PRODUCTION_WORKFLOW_MODE=legacy`，直到 `stepwise-production` 有专用 production activity 和独立 smoke 证据。公司接手 Temporal 分步实现前必须先阅读 `docs/handover/stepwise-production-implementation-task-package-v0.1.md`：当前 `stepwise-recording` 只写 step audit，不是用户实测或生产发布包链路；`stepwise-production` 只有在专用 production activity 和独立 smoke 完成后，才能进入 `READY_LOCAL` 候选。
...
2. 真实写词：单独按 DeepSeek runbook 打开 `AGENT_REAL_CALLS_ENABLED=true` 和 `DEEPSEEK_REAL_CALLS_ENABLED=true`，再通过 `ALLOW_REAL_MODEL_SMOKE=1 ALLOW_DEEPSEEK_REAL_SMOKE=1 TARGET=deepseek MODE=execute scripts/smoke/real-model-controlled-smoke.sh` 跑 1 条写词样本；统一入口默认委托 `scripts/smoke/deepseek-real-lyrics-stack-smoke.sh` 自动启动/清理 API，`scripts/smoke/deepseek-real-lyrics-smoke.sh` 只用于操作者已手动启动 API 的低层调试。
...
每次真实 smoke 后统一更新 `docs/integrations/real-model-smoke-evidence-log.md`，只记录脱敏摘要。完整 provider task id、供应商 URL、签名 URL、完整 Prompt、完整歌词、原始响应和凭据不得进入仓库。Yunwu / WellAPI 公网样本必须与 DreamMaker 生产目标样本分开记录。
...
- 不把 `.env.example` 的 Yunwu / WellAPI 本地联调默认值直接作为生产默认；生产样例必须使用 `deploy/env.production.example` 和 DreamMaker backend。
...
| DeepSeek 一键写词 stack smoke | `scripts/smoke/deepseek-real-lyrics-stack-smoke.sh` |
| DeepSeek 低层写词 smoke | `scripts/smoke/deepseek-real-lyrics-smoke.sh` |
...
- `stepwise-production` 分步生产态推进实现；当前只有 `stepwise-recording` 录步审计，不能作为发布包完成链路。
- 正式前端承接策略。
