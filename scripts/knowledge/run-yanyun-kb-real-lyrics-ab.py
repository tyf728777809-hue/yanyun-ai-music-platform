...
11. 避免廉价古风堆词，不要过度使用“红尘、宿命、刀光剑影、天涯、此生无悔”等空泛表达。
12. 减少“明月、山河、江湖、风烟、长夜、流浪、故乡”等万能诗性词连续堆叠；这些词可以使用，但每段不要当填充词连用，出现时必须落到具体动作、物件或场景上。
13. 如果一段里出现 3 个以上万能词，内部重写替换为更具体的燕云画面、人物动作或生活细节；禁止只靠“酒、剑、月、风”撑完整首歌。
14. 押韵要自然，不为了押韵牺牲内容，不使用生硬倒装、土味口号、网络梗或廉价古风词。
15. 必须原创，不模仿、不改写、不借用现实歌曲歌词、影视台词或已有商业歌词。
16. 不写真实歌手名、现实歌曲名、翻唱导向、仿唱导向。
17. 只输出 JSON object，不输出 Markdown 或解释。
...
4. 可唱性：是否像歌词而不是散文。
5. 副歌记忆点：是否有可传播的 hook。
6. 押韵自然度：副歌是否有主韵脚，整首是否避免完全无韵，且不硬凑。
7. 句长可唱性：中文短句是否利于演唱，是否避免散文化长句。
8. 万能词密度：是否把“明月、山河、江湖、风烟、长夜、流浪、故乡”等当填充词连续堆叠；越低越好。
9. 泛古风污染：越低越好。
