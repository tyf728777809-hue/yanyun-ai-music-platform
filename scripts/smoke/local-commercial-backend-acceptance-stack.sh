...
LOG_DIR="${LOG_DIR:-${REPO_ROOT}/build/smoke/local-commercial-backend-acceptance-$(date +%Y%m%d%H%M%S)}"
...
start_api() {
  ...
  mkdir -p "$LOG_DIR"
  assert_port_free
  log "starting API phase=${phase}; log=${API_LOG}"
...
run_normal_smokes() {
  log "running normal Mock backend smoke suite"
...
run_package_block_smoke() {
  log "running Mock package-block backend smoke"
  MOCK_USER_ID=mock_package_block_smoke \
  API_BASE_URL="${API_ROOT}/api/v1" \
  API_HEALTH_URL="${API_ROOT}/health" \
...
main() {
  ...
  log "backend acceptance smoke is Mock-only; real provider calls stay disabled"
  log "DreamMaker guard remains checked by company-adapter-readiness-smoke.sh"
  ...
  assert_port_free
  log "PASS local commercial backend acceptance stack"
  print_logs_hint
}
