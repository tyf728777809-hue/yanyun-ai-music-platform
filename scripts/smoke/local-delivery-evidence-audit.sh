...
check_production_provider_defaults_audit() {
  local output
  if output="$(scripts/smoke/production-provider-defaults-audit.sh 2>&1)"; then
    pass "production provider defaults audit passes"
    return
  fi

  fail_check "production provider defaults audit failed"
  printf '%s\n' "$output" | tail -40
}
...
require_pattern "README.md" "deepseek-real-lyrics-stack-smoke\\.sh" "README references DeepSeek real lyrics stack smoke"
require_pattern "README.md" "deepseek-real-lyrics-smoke\\.sh" "README references DeepSeek low-level smoke"
