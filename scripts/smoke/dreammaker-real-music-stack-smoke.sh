...
LOG_DIR="${LOG_DIR:-build/smoke/dreammaker-real-music-stack-$(date +%Y%m%d%H%M%S)}"

API_PID=""
WORKER_PID=""

fail() {
  printf '[dreammaker-stack] ERROR: %s\n' "$*" >&2
  print_logs_hint >&2
  exit 1
}

log() {
  printf '[dreammaker-stack] %s\n' "$*"
}
...
port_free() {
  ...
  if lsof -nP -iTCP:"$port" -sTCP:LISTEN >/dev/null 2>&1; then
    fail "port $port is already in use; stop existing service or use dreammaker-real-music-smoke.sh against it manually"
  fi
}
...
main() {
  ...
  start_api
  run_smoke
  log "PASS stack smoke provider=$REAL_PROVIDER"
  print_logs_hint
}
