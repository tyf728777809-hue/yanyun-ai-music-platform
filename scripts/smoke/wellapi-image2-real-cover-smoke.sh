...
IDEMPOTENCY_PREFIX="${IDEMPOTENCY_PREFIX:-wellapi-image2-real-cover-$(date +%s)}"
...
if [ "${IMAGE_REAL_CALLS_ENABLED:-false}" != "true" ]; then
  fail "IMAGE_REAL_CALLS_ENABLED must be true for real WellAPI Image2 smoke."
fi

if [ "${IMAGE2_BACKEND:-wellapi}" != "wellapi" ]; then
  fail "IMAGE2_BACKEND must be wellapi for this smoke."
fi
...
IMAGE2_STATUS="$(
  echo "$READINESS" | jq -r '.components[] | select(.component == "image2_guard") | .status'
)"
IMAGE2_MODE="$(
  echo "$READINESS" | jq -r '.components[] | select(.component == "image2_guard") | .configured_mode'
)"
MUSIC_MODE="$(
  echo "$READINESS" | jq -r '.components[] | select(.component == "music_provider") | .configured_mode'
)"

if [ "$IMAGE2_STATUS" != "READY_FOR_LOCAL" ] || [ "$IMAGE2_MODE" != "real-calls-enabled/wellapi" ]; then
  echo "$READINESS" | jq '.components[] | select(.component == "image2_guard")' >&2
  fail "image2_guard is not READY_FOR_LOCAL real-calls-enabled/wellapi"
fi
...
log "creating one lyrics work for real WellAPI Image2 cover smoke"
CREATE_RESPONSE="$(
  post_json "/works/lyrics" "$IDEMPOTENCY_PREFIX-create" '{
    "song_title": "WellAPI Image2 cover smoke",
...
log "confirming work with mock music and real WellAPI Image2 cover"
CONFIRM_RESPONSE="$(
...
if [ -z "$FINAL_DETAIL" ]; then
  fail "real WellAPI Image2 smoke timed out. work_id=$WORK_ID"
fi
...
if [ "$COVER_PROVIDER" != "wellapi-image2" ] || [ "$META_PROVIDER" != "wellapi-image2" ]; then
...
if [ "$FINAL_STATUS" = "GENERATED" ] && [ "$PACKAGE_STATUS" = "PACKAGE_READY" ]; then
  ...
  log "PASS real WellAPI Image2 cover smoke. work_id=$WORK_ID"
  exit 0
fi

log "real WellAPI Image2 smoke finished without PACKAGE_READY. work_id=$WORK_ID"
exit 1
