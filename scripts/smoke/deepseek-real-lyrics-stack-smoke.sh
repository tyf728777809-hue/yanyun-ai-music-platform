...
start_api() {
  ...
  DEEPSEEK_CREATIVE_BRIEF_TIMEOUT="${DEEPSEEK_CREATIVE_BRIEF_TIMEOUT:-20s}" \
  DEEPSEEK_MAX_ATTEMPTS="${DEEPSEEK_MAX_ATTEMPTS:-1}" \
  DEEPSEEK_RESPONSE_MAX_TOKENS="${DEEPSEEK_RESPONSE_MAX_TOKENS:-4096}" \
  DEEPSEEK_CREATIVE_BRIEF_RESPONSE_MAX_TOKENS="${DEEPSEEK_CREATIVE_BRIEF_RESPONSE_MAX_TOKENS:-900}" \
  DEEPSEEK_CREATIVE_BRIEF_SEMANTIC_ATTEMPTS="${DEEPSEEK_CREATIVE_BRIEF_SEMANTIC_ATTEMPTS:-1}" \
  DEEPSEEK_LYRICS_SEMANTIC_ATTEMPTS="${DEEPSEEK_LYRICS_SEMANTIC_ATTEMPTS:-2}" \
  DEEPSEEK_TEMPERATURE="${DEEPSEEK_TEMPERATURE:-0.7}" \
  LYRICS_FIRST_DRAFT_MODE="${LYRICS_FIRST_DRAFT_MODE:-conditional-brief}" \
  LYRICS_QUALITY_GATE_MODE="${LYRICS_QUALITY_GATE_MODE:-self-score-only}" \
  MUSIC_PROVIDER=mock \
  IMAGE_PROVIDER=mock \
